...
import { Play, Square, Users } from 'lucide-react-native'
...
interface ExamAudioProps {
  /** Chuỗi (một giọng) hoặc mảng lượt nói (hai giọng — Hörverstehen Teil 2 của telc). */
  script: string | Turn[]
  label: string
 ...
 * chuyện có thật, và Hörverstehen Teil 2 là 10 câu trên 25 điểm. Lời thoại thì CỐ Ý không in:
 * đây là bài nghe.
 */
export function ExamAudio({ script, label, maxPlays }: ExamAudioProps) {
  ...
  const turns: Turn[] = Array.isArray(script) ? script.filter((t) => t.text?.trim()) : []
  const isDialogue = turns.length > 0
  ...
  const play = useCallback(async () => {
    if (!canPlayAgain(maxPlays, used)) return
    setUsed((n) => n + 1)
    setPlaying(true)
    try {
      if (isDialogue) {
  ...
  }, [isDialogue, turns, script, maxPlays, used])

  return (
    <View style={{ gap: space[3], backgroundColor: c.infoSoft, borderRadius: radius.md, padding: space[3] }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: space[3] }}>
        <Pressable
          ...
          style={{
            ...
            backgroundColor: exhausted && !playing ? c.border : c.accent,
          }}
        >
          <Icon icon={playing ? Square : Play} size={18} color="onAccent" />
        </Pressable>
        <View style={{ flex: 1, gap: 2 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: space[2] }}>
            {isDialogue ? <Icon icon={Users} size={14} color="info" /> : null}
            <ThemedText variant="bodyStrong" color="info">{label}</ThemedText>
          </View>
          <Caption color={c.info}>
            {exhausted
          ...
          {turns.map((turn, idx) => (
            <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', gap: space[2] }}>
              <ThemedText variant="caption" color={activeLine === idx ? 'info' : 'faint'}>
                {turn.name ?? (idx % 2 === 0 ? 'Người 1' : 'Người 2')}
              </ThemedText>
