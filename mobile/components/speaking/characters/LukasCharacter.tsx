// Lukas — RN port of the web bespoke SVG character (1:1). Pure react-native-svg +
// React state (lip-sync mouth + idle blink). No external animation deps.

import { useEffect, useState } from 'react'
import Svg, { Path, Ellipse, Rect, Circle, Line, Text as SvgText, G } from 'react-native-svg'

export type CharExpression = 'neutral' | 'talking' | 'smiling' | 'thinking' | 'serious'
...
function Eyebrows({ expression }: { expression: CharExpression }) {
  const c = '#1A1A2E'
  const sw = 5.5
  switch (expression) {
    case 'smiling':
...
function Eyes({ blinking, expression }: { blinking: boolean; expression: CharExpression }) {
  const squint = expression === 'smiling'
  if (blinking) {
    return (
      <G>
        <Rect x={106} y={173} width={36} height={5} rx={2.5} fill={SKIN} />
        <Rect x={146} y={173} width={36} height={5} rx={2.5} fill={SKIN} />
      </G>
    )
  }
  return (
...
function MouthFrame({ frame, expression }: { frame: number; expression: CharExpression }) {
  ...
  if (frame === 0) {
    return (
      <G>
        ...
        <Ellipse cx={144} cy={213} rx={18} ry={5} fill={SKIN} />
      </G>
    )
  }
  if (frame === 1) {
    return (
      <G>
        ...
        <Ellipse cx={144} cy={215} rx={13} ry={3.5} fill={SKIN} />
      </G>
    )
  }
...
export function LukasCharacter({ expression = 'neutral', isTalking = false, size, paused = false }: Props) {
  ...
  return (
    <Svg width={size ?? '100%'} height={size ?? '100%'} viewBox="0 0 280 500">
      ...
      <SvgText x={100} y={344} fontSize={8} fill="#3D5A75" fontFamily="monospace" letterSpacing={4} opacity={0.75}>01 10</SvgText>
      <SvgText x={100} y={358} fontSize={8} fill="#3D5A75" fontFamily="monospace" letterSpacing={4} opacity={0.75}>11 00</SvgText>
      <SvgText x={100} y={372} fontSize={8} fill="#3D5A75" fontFamily="monospace" letterSpacing={4} opacity={0.75}>10 01</SvgText>

      <Circle cx={202} cy={348} r={17} fill="#E87600" fillOpacity={0.06} />
      <SvgText x={195} y={353} fontSize={13} opacity={0.65}>☕</SvgText>
      ...
      <Path d="M 110 264 C 116 259 127 255 144 255 C 161 255 172 259 178 264" fill="none" stroke="#263C50" strokeWidth={3} strokeLinecap="round" opacity={0.5} />
      ...
      {expression === 'thinking' ? (
        <G>
          <Path d="M 36 310 C 52 296 74 274 96 268" fill="none" stroke={SHIRT} strokeWidth={32} strokeLinecap="round" />
          <Ellipse cx={96} cy={263} rx={20} ry={16} fill={SKIN} />
          <Path d="M 84 252 C 82 257 84 264 88 266" fill="none" stroke={SKIN_D} strokeWidth={2} strokeLinecap="round" opacity={0.45} />
          <Path d="M 94 248 C 92 254 94 261 98 263" fill="none" stroke={SKIN_D} strokeWidth={2} strokeLinecap="round" opacity={0.45} />
        </G>
      ) : null}
    </Svg>
  )
