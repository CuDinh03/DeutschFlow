...
- ✅ **CRASH KHI MỞ APP — ĐÃ FIX** (commit `660c753a`). Root cause = redirect imperative (`router.replace` trong effect phụ thuộc `useRootNavigationState()`) → vòng lặp re-render vô hạn ở `@react-navigation/core` useSyncState. Đã chuyển sang `<Redirect>` khai báo ở `app/index.tsx` + bỏ `useRootNavigationState` khỏi root layout. **Verified trên iOS Simulator (dev build): app mở tới màn login, KHÔNG còn loop.** Chi tiết: mục dưới.
- ⏳ Còn 8 màn chưa restyle + nợ migrate `expo-av`. Next = **rebuild EAS preview** xác nhận release build + làm nốt reskin.
