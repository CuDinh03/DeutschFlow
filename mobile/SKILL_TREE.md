...
| **Stage tabs** | `StageTabs` in `roadmap.tsx` → `buildTreeLayout(preview)` | Maturity-PREVIEW segmented control (Mầm / Hiện tại / Cây lớn). `preview` is a **render-only override** of level classification: `mam` forces every level locked (a bare sprout), `big` forces every level passed (full grown tree + gold discs), `current` is real progress. Never changes data/gating. |
...
- **Design-match deferred (2026-06-30, see SKILL_TREE_PROGRESS.md §12):** the new design's **"Lên cấp" ritual button** (needs a backend level-up action + the H4 ready signal — not added rather than ship a dead button) and **per-skill branch labels** (Đọc/Viết/Nghe/Nói — needs real per-node skill data; kept real topic labels instead of faking). The faint top **crown could become a dashed goal-dot** to match the mockup exactly (cosmetic). The mầm sprout, gold milestone discs, and maturity stage tabs ARE shipped.
