 ...
 *
 * <p>Taxonomy scope = tags where {@code is_topic_taxonomy} is true.
 */
@Service
...
public class VocabularyAutoTaggingService {
    ...
    public Map<String, Object> runBatch(long actorUserId, Integer limit, boolean dryRun, boolean resetTags) {
        ...
        if (taxonomy.isEmpty()) {
            return Map.of(
                    "status", "error",
                    "message", "No topic taxonomy tags (is_topic_taxonomy). Run V31+V46 migrations.");
        }
    ...
    private List<Map<String, Object>> fetchUntaggedWords(int limit) {
        String sql = """
                ...
                WHERE NOT EXISTS (
                    ...
                    JOIN tags t2 ON t2.id = wt2.tag_id AND t2.is_topic_taxonomy IS TRUE
    ...
    private void clearTopicTaxonomyLinks() {
        jdbc.update("""
                DELETE FROM word_tags wt
                USING tags t
                WHERE t.id = wt.tag_id AND t.is_topic_taxonomy IS TRUE
                """);
    }
     ...
     */
    /**
     * LLM tagging uses the app's primary chat LLM ({@link com.deutschflow.speaking.ai.OpenAiChatClient}).
    ...
    protected void applyTags(long wordId, List<String> tagNames, List<String> taxonomy) {
        ...
        for (String tagName : tagNames) {
            ...
            Long tagId = jdbc.query(
                    ...
                    tagName);
            if (tagId == null) {
                jdbc.update("INSERT INTO tags (name) VALUES (?) ON CONFLICT (name) DO NOTHING", tagName);
                tagId = jdbc.queryForObject("SELECT id FROM tags WHERE name = ?", Long.class, tagName);
            }
            if (tagId == null) continue;
            jdbc.update(
                    "INSERT INTO word_tags (word_id, tag_id) VALUES (?, ?) ON CONFLICT DO NOTHING",
                    wordId, tagId);
        }
    }
}
