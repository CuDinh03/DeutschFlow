        ...
        for (String raw : body.split("\\R")) {
            ...
            if ("Noun".equals(dtype) && gender != null) {
                ...
                jdbcTemplate.update(
                        ...
                        ON CONFLICT (id) DO UPDATE SET
                          ...
                          plural_form = CASE WHEN nouns.plural_form IS NULL OR TRIM(nouns.plural_form) = '' THEN EXCLUDED.plural_form ELSE nouns.plural_form END,
                          genitive_form = CASE WHEN nouns.genitive_form IS NULL OR TRIM(nouns.genitive_form) = '' THEN EXCLUDED.genitive_form ELSE nouns.genitive_form END
                        """,
            ...
            if ("Verb".equals(dtype)) {
                jdbcTemplate.update(
                        ...
                        VALUES (?, 'HABEN', 'n/a', ?, ?, FALSE)
                        ON CONFLICT (id) DO UPDATE SET
                          is_separable = CASE WHEN EXCLUDED.is_separable IS TRUE THEN TRUE ELSE verbs.is_separable END,
                          prefix = CASE WHEN EXCLUDED.prefix IS NOT NULL AND (verbs.prefix IS NULL OR TRIM(verbs.prefix) = '') THEN EXCLUDED.prefix ELSE verbs.prefix END
                        """,
                        wordId,
                        parsed.isSeparable(),
                        parsed.verbPrefix()
