...
public class VocabularyResetService {
    ...
    public Map<String, Object> resetAndReimport(int wiktionaryBatchLimit) {
        ...
        jdbcTemplate.update("ALTER TABLE words AUTO_INCREMENT = 1");
        ...
        jdbcTemplate.update("""
                UPDATE words SET base_form = TRIM(SUBSTRING_INDEX(base_form, ',', 1))
                WHERE base_form LIKE '%,%'
                """);
        // Extract gender from article prefix and store in nouns table
        jdbcTemplate.update("""
                INSERT IGNORE INTO nouns (id, gender, noun_type)
                SELECT w.id,
                  CASE
                    WHEN LOWER(w.base_form) REGEXP '^der ' THEN 'DER'
                    WHEN LOWER(w.base_form) REGEXP '^die ' THEN 'DIE'
                    WHEN LOWER(w.base_form) REGEXP '^das ' THEN 'DAS'
                  END,
                  'STARK'
                FROM words w
        ...
        jdbcTemplate.update("""
                UPDATE words SET base_form = TRIM(REGEXP_REPLACE(base_form, '^(der|die|das) ', ''))
                WHERE LOWER(base_form) REGEXP '^(der|die|das) '
                """);
        // Remove duplicates (keep lowest id per base_form case-insensitive)
        jdbcTemplate.update("""
                DELETE FROM words WHERE id NOT IN (
                  SELECT keep_id FROM (
                    SELECT MIN(id) as keep_id FROM words GROUP BY LOWER(base_form)
                  ) t
                )
                """);
