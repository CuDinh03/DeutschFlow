...
public class AiResponseParser {
     ...
     * Defaults to contract {@link SpeakingResponseSchema#V1}.
     */
    public AiParseOutcome parseWithOutcome(String rawJson) {
        return parseWithOutcome(rawJson, SpeakingResponseSchema.V1);
    }

    /**
     * Parses using the session's response contract (V1 canonical tutor JSON vs V2 compact JSON).
     */
    public AiParseOutcome parseWithOutcome(String rawJson, SpeakingResponseSchema schema) {
        if (rawJson == null) {
            return new AiParseOutcome(fallback(""), AiParseStatus.FALLBACK_NULL_INPUT);
        }
        if (schema == SpeakingResponseSchema.V2) {
            return parseV2WithOutcome(rawJson);
        }
        return parseV1WithOutcome(rawJson);
    }

    private AiParseOutcome parseV1WithOutcome(String rawJson) {
        ...
        try {
            ...
            JsonNode simNode = root.get("similarity_score");
            if (simNode != null && simNode.isNumber()) {
                similarityScore = simNode.asDouble();
            }
            String feedback = textOrNull(root, "feedback");
            List<AiResponseDto.AiSuggestionDto> suggestions = parseSuggestionsArray(root.get("suggestions"));
            AiResponseDto.InterviewMeta interviewMeta = parseInterviewMeta(root.get("interview_meta"));
    ...
    private AiParseOutcome parseV2WithOutcome(String rawJson) {
        String cleaned = extractJsonObject(stripMarkdownFences(rawJson.trim()));
        ...
        try {
            ...
            String translation = textOrNull(root, "translation");
            String feedback = textOrNull(root, "feedback");
    ...
    /**
     * Parses suggestions JSON array.
     */
    private AiResponseDto.InterviewMeta parseInterviewMeta(JsonNode node) {
    ...
    private List<AiResponseDto.AiSuggestionDto> parseSuggestionsArray(JsonNode suggestionsNode) {
        ...
        List<AiResponseDto.AiSuggestionDto> out = new ArrayList<>();
        for (JsonNode item : suggestionsNode) {
            if (item == null || item.isNull()) continue;
            out.add(new AiResponseDto.AiSuggestionDto(
