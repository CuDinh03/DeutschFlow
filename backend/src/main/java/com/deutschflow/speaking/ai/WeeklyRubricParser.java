...
public class WeeklyRubricParser {
    ...
    private WeeklySpeakingDtos.WeeklyRubricView parseFromRoot(JsonNode root) {
        ...
        List<WeeklySpeakingDtos.WeeklyRubricView.ReplacementSuggestion> sug = parseVocab(lx.get("replacements_suggested_de_vi"));

        JsonNode gr = required(root, "grammar");
        String gsum = textOrBlank(gr, "summary_de");
        List<WeeklySpeakingDtos.WeeklyGrammarErrorHint> errs = parseGrammarErrors(gr.get("errors"));
        ...
        return new WeeklySpeakingDtos.WeeklyRubricView(
                new WeeklySpeakingDtos.WeeklyRubricView.TaskCompletion(score, covered, missing, offTopic, ambiguous),
                new WeeklySpeakingDtos.WeeklyRubricView.Fluency(fluNotes, filler, wpm == null ? null : wpm.intValue(), conf.isBlank() ? "TEXT_ONLY_PROXY" : conf),
                new WeeklySpeakingDtos.WeeklyRubricView.Lexis(rich, sug),
                new WeeklySpeakingDtos.WeeklyRubricView.Grammar(gsum, errs),
                fb,
                dis
        );
    }
    ...
    private List<WeeklySpeakingDtos.WeeklyRubricView.ReplacementSuggestion> parseVocab(JsonNode arr) {
        List<WeeklySpeakingDtos.WeeklyRubricView.ReplacementSuggestion> out = new ArrayList<>();
        if (arr == null || !arr.isArray()) return out;
        for (JsonNode n : arr) {
            ...
            if (from != null || to != null) {
                out.add(new WeeklySpeakingDtos.WeeklyRubricView.ReplacementSuggestion(
                        from == null ? "" : from,
                        to == null ? "" : to,
                        nv == null ? "" : nv
                ));
    ...
    private List<WeeklySpeakingDtos.WeeklyGrammarErrorHint> parseGrammarErrors(JsonNode arr) {
        List<WeeklySpeakingDtos.WeeklyGrammarErrorHint> out = new ArrayList<>();
        if (arr == null || !arr.isArray()) return out;
        int cap = 0;
        for (JsonNode item : arr) {
            ...
            out.add(new WeeklySpeakingDtos.WeeklyGrammarErrorHint(
                    code.trim(),
                    sev,
                    conf,
