...
import com.deutschflow.speaking.dto.ImportReviewErrorsRequest;
...
public class ReviewTasksController {
    ...
    @PostMapping("/import")
    public ResponseEntity<Void> importErrors(
            @AuthenticationPrincipal User user,
            @Valid @RequestBody ImportReviewErrorsRequest body) {
        reviewSchedulerService.importReviewErrors(user.getId(), body.errors(), "REWRITE");
