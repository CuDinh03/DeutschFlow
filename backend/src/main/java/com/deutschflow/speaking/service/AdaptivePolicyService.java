...
public class AdaptivePolicyService {
    ...
    public void recordCooldownAfterRepair(Long userId, String errorCode) {
        if (!adaptiveEnabled || errorCode == null || errorCode.isBlank()) {
            return;
        }
        SpeakingUserState state = getOrCreateState(userId);
        ...
        entries.removeIf(e -> e.code().equalsIgnoreCase(errorCode.trim()));
        entries.add(new CooldownEntry(errorCode.trim(), until.format(COOLDOWN_FMT)));
