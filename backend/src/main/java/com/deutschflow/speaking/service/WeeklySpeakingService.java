...
public class WeeklySpeakingService {
    ...
    public WeeklySpeakingDtos.WeeklyPromptResponse getCurrentPrompt(long userId, String cefrParam) {
        ...
        Map<String, Object> row = loadPromptRowWithAdjacentBandFallback(weekStart, band);
        if (row == null) {
    ...
    /**
     * If no prompt exists for {@code resolvedBand}, try the nearest bands on the CEFR ladder:
     * first easier (below), then harder (above). Avoids misleading fallbacks such as jumping to B1 when the
     * learner asks for A1 but only seeded rows existed with unstable {@code ORDER BY id} ordering.
     */
    private Map<String, Object> loadPromptRowWithAdjacentBandFallback(LocalDate weekStart, String resolvedBand) {
        Map<String, Object> row = loadPromptRow(weekStart, resolvedBand);
        if (row != null) {
            return row;
        }
        int bi = SpeakingCefrSupport.bandIndex(resolvedBand);
        if (bi < 0) {
            bi = 0;
        }
        for (int down = bi - 1; down >= 0; down--) {
            row = loadPromptRow(weekStart, SpeakingCefrSupport.bandFromLadderIndex(down));
            if (row != null) {
                return row;
            }
        }
        for (int up = bi + 1; up < SpeakingCefrSupport.ladderStepCount(); up++) {
            row = loadPromptRow(weekStart, SpeakingCefrSupport.bandFromLadderIndex(up));
            if (row != null) {
                return row;
            }
        }
