        ...
        try {
            ...
            if (existing != null && existing > 0) {
                String raw = jdbcTemplate.queryForObject("""
                                SELECT CAST(metrics_json AS TEXT) FROM learner_progress_reports
                                WHERE user_id = ? AND period_type = 'WEEK' AND period_start = ?
                                """,
                ...
                jdbcTemplate.update("""
                                ...
                                SET metrics_json = CAST(? AS JSONB), updated_at = CURRENT_TIMESTAMP
                                WHERE user_id = ? AND period_type = 'WEEK' AND period_start = ?
                                """,
                ...
                jdbcTemplate.update("""
                                ...
                                ) VALUES (?, 'WEEK', ?, ?, 'Asia/Ho_Chi_Minh', 'DRAFT', CAST(? AS JSONB))
                                """,
