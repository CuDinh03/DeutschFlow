...
public class SkillTreeService {
    ...
    public List<Map<String, Object>> getSkillTreeForUser(long userId) {
        ...
        return jdbcTemplate.queryForList("""
                SELECT
                    ...
                    n.node_code, n.node_family, n.mastery_threshold, n.unlock_rule,
    ...
    private void autoUnlockFirstNodeIfNeeded(long userId) {
        ...
        List<Map<String, Object>> firstNodes = jdbcTemplate.queryForList("""
                ...
                WHERE is_active = TRUE AND node_type = 'CORE_TRUNK'
                  AND (node_family = 'FOUNDATION' OR phase = 'FOUNDATION')
                ORDER BY COALESCE(day_number, 9999) ASC, sort_order ASC
                LIMIT 1
                """);
    ...
    public Map<String, Object> getNodeSession(long userId, long nodeId) {
        ...
        result.put("nodeCode", node.get("node_code"));
        result.put("nodeFamily", node.get("node_family"));
        result.put("masteryThreshold", node.get("mastery_threshold"));
        result.put("unlockRule", node.get("unlock_rule"));
        result.put("estimatedMinutes", node.get("estimated_minutes"));
        result.put("contentKey", node.get("content_key"));
