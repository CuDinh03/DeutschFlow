        ...
        switch (part.flow()) {
            case EXAMINER_LED -> {
                ...
                if (spelling) {
                    steps.add(new SessionPlan.Step(i++, "SPEAK", 0, "PRUEFER", "SPELL_REQUEST",
                            "Giới thiệu bản thân theo các từ khóa trên thẻ (tên, tuổi, nước, nơi ở, ngôn ngữ, nghề, sở thích).", "INTRO_SPELL"));
                    steps.add(new SessionPlan.Step(i++, "SPEAK", 0, "PRUEFER", "NUMBER_REQUEST", "Đánh vần từ giám khảo yêu cầu (buchstabieren).", "SPELL_WORD"));
                    steps.add(new SessionPlan.Step(i++, "SPEAK", 0, "PRUEFER", "THANK", "Đọc số/điện thoại/mã bưu điện giám khảo yêu cầu.", "READ_NUMBER"));
                } else {
                    int n = Math.max(2, part.maxCandidateTurns());
                    steps.add(new SessionPlan.Step(i++, "SPEAK", 0, "PRUEFER", "FOLLOWUP_QUESTION",
                            "Giới thiệu bản thân theo các từ khóa trên thẻ — nói thành câu đầy đủ.", "INTRO_CARD"));
                    for (int k = 1; k < n; k++) {
                        steps.add(new SessionPlan.Step(i++, "ANSWER", 0, "PRUEFER", k == n - 1 ? "THANK" : "FOLLOWUP_QUESTION",
                                "Trả lời câu hỏi thêm của giám khảo.", "ANSWER_FOLLOWUP"));
                    }
                }
            }
            case ALTERNATING_QA -> {
                ...
                String askKey = switch (firstType) {
                    case "PERSON_CARD" -> "ASK_PERSON";
                    case "QUESTION_WORD_CARD" -> "ASK_QWORD";
                    default -> request ? "ASK_REQUEST" : "ASK_CARD";
                };
                ...
                String answerKey = request ? "REACT_REQUEST" : "ANSWER_PARTNER";
                for (int r = 0; r < rounds; r++) {
                    int askCard = Math.min(2 * r, Math.max(0, cards - 1));
                    int answerCard = Math.min(2 * r + 1, Math.max(0, cards - 1));
                    steps.add(new SessionPlan.Step(i++, "ASK", askCard, "PARTNER", "ANSWER_AND_ASK", askHint, askKey));
                    steps.add(new SessionPlan.Step(i++, "ANSWER", answerCard, "PARTNER", "REACT", answerHint, answerKey));
                }
            }
            case MONOLOGUE -> {
                ...
                steps.add(new SessionPlan.Step(i++, "SPEAK", 0,
                        ...
                        presentation ? "PRESENT_LONG" : "PRESENT_SHORT"));
                for (int k = 1; k < n; k++) {
                    boolean last = k == n - 1;
                    ...
                    steps.add(new SessionPlan.Step(i++, "ANSWER", 0, role, last ? "THANK" : "FOLLOWUP_QUESTION",
                            ...
                            presentation && k == 1 ? "ANSWER_PARTNER_PRESENTATION" : "ANSWER_QUESTION"));
                }
            }
            case DIALOGUE -> {
                int n = Math.max(2, part.maxCandidateTurns());
                ...
                boolean vorlage = "TOPIC_GRAPHIC_PAIR".equals(firstType) || "TOPIC_TEXT_PAIR".equals(firstType);
                ...
                for (int k = 0; k < n; k++) {
                    ...
                    if (contact && k == n - 2) {
                        ...
                        steps.add(new SessionPlan.Step(i++, "REACT", 0, "PRUEFER", "FOLLOWUP_QUESTION",
                                "Phản hồi bạn thi; sau đó giám khảo sẽ hỏi thêm một câu.", "CONTACT_THEN_PRUEFER"));
                        continue;
                    }
                    if (contact && last) {
                    ...
                    String aiAction = last ? "CONCLUDE" : (vorlageInfo ? "REPORT_OWN" : "REACT_AND_ASK");
            ...
            case FEEDBACK -> {
                ...
                steps.add(new SessionPlan.Step(i++, "SPEAK", 0, "PARTNER", "ANSWER_QUESTION",
                        ...
                        "FEEDBACK_COMMENT", "PRUEFER", "FOLLOWUP_QUESTION"));
                steps.add(new SessionPlan.Step(i++, "ANSWER", 0, "PRUEFER", "THANK", "Trả lời câu hỏi của giám khảo về chủ đề.", "ANSWER_PRUEFER_TOPIC"));
            }
        }
        return steps;
    }
