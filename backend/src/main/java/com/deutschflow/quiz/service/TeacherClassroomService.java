...
public class TeacherClassroomService {
    ...
    public void addStudent(Long teacherId, Long classId, String studentEmail) {
        ...
        payload.put("studentDisplayName", student.getDisplayName());
        payload.put("teacherName", teacherId != null ? userRepository.findById(teacherId).map(User::getDisplayName).orElse("") : "");
        payload.put("action", "ADDED");
        userNotificationService.insertForUser(
                userRepository.findById(teacherId).orElseThrow(() -> new NotFoundException("Teacher not found")),
    ...
    public void removeStudent(Long teacherId, Long classId, Long studentId) {
        ...
        userNotificationService.insertForUser(
                userRepository.findById(teacherId).orElseThrow(() -> new NotFoundException("Teacher not found")),
                NotificationType.CLASS_STUDENT_REMOVED,
                payload
        );
    }
