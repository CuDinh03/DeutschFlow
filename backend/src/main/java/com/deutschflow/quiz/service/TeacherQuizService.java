...
public class TeacherQuizService {
    ...
    public Map<String, Object> createQuiz(Long teacherId, String title, String quizType, Long classroomId) {
        ...
        Long id = jdbcTemplate.queryForObject("""
                ...
                VALUES (?, ?, ?, ?, ?, 'DRAFT', ?)
                RETURNING id
                """, Long.class, teacherId, classroomId, normalizedTitle, quizType, pin, Timestamp.valueOf(LocalDateTime.now()));
    ...
    public void addQuestion(Long teacherId, Long quizId, String question, Integer timeLimit, Integer position, List<Map<String, Object>> choices) {
        ...
        Long questionId = jdbcTemplate.queryForObject("""
                ...
                VALUES (?, ?, ?, ?)
                RETURNING id
                """, Long.class, quizId, question.trim(), normalizedTime, normalizedPosition);
    ...
    public void updateStatus(Long teacherId, Long quizId, String status) {
        ...
        if ("WAITING".equals(status)) {
            Map<String, Object> quizInfo = jdbcTemplate.query("""
                SELECT q.status, q.title, q.classroom_id, c.name as classroom_name, u.display_name as teacher_name
                FROM quizzes q
                ...
                map.put("classroomId", rs.getObject("classroom_id"));
                map.put("classroomName", rs.getString("classroom_name"));
                map.put("teacherName", rs.getString("teacher_name"));
                return map;
            }, quizId);
            
            if (quizInfo != null && "DRAFT".equals(quizInfo.get("status")) && quizInfo.get("classroomId") != null) {
                Long classroomId = ((Number) quizInfo.get("classroomId")).longValue();
                String title = (String) quizInfo.get("title");
                String classroomName = (String) quizInfo.get("classroomName");
                String teacherName = (String) quizInfo.get("teacherName");
                
                eventPublisher.publishEvent(new com.deutschflow.notification.events.QuizAssignedEvent(
                        classroomId, quizId, title, teacherName, classroomName
                ));
            }
        }
