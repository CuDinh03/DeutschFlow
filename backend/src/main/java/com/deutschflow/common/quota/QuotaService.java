...
public class QuotaService {
    ...
    public void applyUsageDebit(long userId, long totalTokens, Instant now) {
        ...
        jdbcTemplate.update(
                """
                        UPDATE user_ai_token_wallets SET balance = GREATEST(0, balance - ?), updated_at = CURRENT_TIMESTAMP(6)
                        WHERE user_id = ?
                        """,
                totalTokens, userId);
    ...
    private void reconcileSubscriptions(long userId, Instant now) {
        jdbcTemplate.update("""
                        ...
                        SET ends_at = TIMESTAMPADD(DAY, 7, starts_at),
    ...
    private void endSubscriptionsPaid(long userId, Instant now) {
        jdbcTemplate.update("""
                        UPDATE user_subscriptions
                        SET status = 'ENDED', ends_at = ?, updated_at = CURRENT_TIMESTAMP(6)
                        WHERE user_id = ?
                          AND status = 'ACTIVE'
    ...
    private void accrueWalletThroughToday(long userId, SubscriptionRow sub, long walletCap, Instant now) {
        ...
        if (walletCap <= 0L || daily <= 0L || spanDays <= 0L) {
            jdbcTemplate.update("""
                            ...
                            SET balance = ?, last_accrual_local_date = ?, updated_at = CURRENT_TIMESTAMP(6)
                            WHERE user_id = ?
                            """,
        ...
        if (balance >= walletCap) {
            ...
            balance = Math.min(walletCap, balance + pulls * daily);
        }
        ...
        jdbcTemplate.update("""
                        ...
                        WHERE user_id = ?
                        """,
                balance, Date.valueOf(today), userId);
    }
