    ...
    public List<Map<String, Object>> latencyErrorSummary(int days) {
        ...
        return jdbcTemplate.query("""
                SELECT
                  ...
                  SUM(CASE WHEN is_error = 1 THEN 1 ELSE 0 END) AS errorRequests,
                  ROUND(AVG(latency_ms), 2) AS avgLatencyMs,
                  ROUND(100.0 * SUM(CASE WHEN is_error = 1 THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0), 2) AS errorRatePercent
