...
class OnboardingTypeResolverTest {
    ...
    @ParameterizedTest(name = "[{index}] {0} × {1} → {2} / {6}")
    @MethodSource("matrix")
    ...
    void matrixCells(Platform platform, CurrentLevel level, OnboardingType type,
                     boolean placement, boolean hook, boolean paywall, PostOnboardingAction action) {
    ...
    private static Stream<Arguments> matrix() {
        return Stream.of(
                // WEB
                Arguments.of(Platform.WEB, CurrentLevel.A0, ZERO_START,          false, false, true, ROADMAP_ALPHABET),
                Arguments.of(Platform.WEB, CurrentLevel.A1, PLACEMENT_VALIDATED, true,  true,  true, ROADMAP_NODE),
                Arguments.of(Platform.WEB, CurrentLevel.A2, PLACEMENT_VALIDATED, true,  true,  true, ROADMAP_NODE),
                Arguments.of(Platform.WEB, CurrentLevel.B1, PLACEMENT_VALIDATED, true,  true,  true, PRICING_CTA),
                Arguments.of(Platform.WEB, CurrentLevel.B2, PLACEMENT_VALIDATED, true,  true,  true, PRICING_CTA),
                // ANDROID
                Arguments.of(Platform.ANDROID, CurrentLevel.A0, ZERO_START,      false, false, true, ROADMAP_ALPHABET),
                Arguments.of(Platform.ANDROID, CurrentLevel.A1, EXPRESS_PROFILE, false, true,  true, MOCK_HOOK_PAYWALL),
                Arguments.of(Platform.ANDROID, CurrentLevel.A2, EXPRESS_PROFILE, false, true,  true, MOCK_HOOK_PAYWALL),
                Arguments.of(Platform.ANDROID, CurrentLevel.B1, ASSESSMENT_HOOK, false, true,  true, RADAR_CHECKOUT),
                Arguments.of(Platform.ANDROID, CurrentLevel.B2, ASSESSMENT_HOOK, false, true,  true, RADAR_CHECKOUT),
                // IOS (paywall always false)
                Arguments.of(Platform.IOS, CurrentLevel.A0, MENTOR_LED_DEMO, false, false, false, EMAIL_CAPTURE_UPSELL),
                Arguments.of(Platform.IOS, CurrentLevel.A1, EXPRESS_PROFILE, false, false, false, START_PRACTICE),
                Arguments.of(Platform.IOS, CurrentLevel.A2, EXPRESS_PROFILE, false, false, false, START_PRACTICE),
                Arguments.of(Platform.IOS, CurrentLevel.B1, EXPRESS_PROFILE, false, false, false, INTERVIEW_FIRST),
                Arguments.of(Platform.IOS, CurrentLevel.B2, EXPRESS_PROFILE, false, false, false, INTERVIEW_FIRST)
        );
    }
    ...
    @DisplayName("A0 never triggers a placement test on any platform")
    void a0NeverPlacement(CurrentLevel level) {
        for (Platform p : Platform.values()) {
            assertThat(resolver.resolve(p, level).placementRequired())
                    .as("placement for %s/%s", p, level)
                    .isFalse();
        }
    }

    @Test
    @DisplayName("placement test is web-only (never on android/iOS)")
    void placementIsWebOnly() {
