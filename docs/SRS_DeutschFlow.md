...
13. **Redis Resilience — Login 500 + Health 503 (PR #114)** — 300ms Redis timeout + loại redis khỏi health (best-effort, giống mail) + regression test. ⚠️ PENDING deploy-backend.sh.
...
| **Redis Resilience / Login** | ⚠️ PENDING deploy | PR #114; login 500 + health 503 fix; `deploy-backend.sh` cũng dựng lại deutschflow-redis |
