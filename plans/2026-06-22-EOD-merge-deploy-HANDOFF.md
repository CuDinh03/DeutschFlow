...
- **Speaking WIP (4 file) đang STASHED** (`git stash list` → `wip-during-149-deploy`): `speaking/chat/page.tsx`, `v2/student/speaking/page.tsx`, `CompanionSelect.tsx`, `useChatStore.ts`. **KHÔNG phải việc của track teacher** — là WIP đồng thời (returnPath/speaking). Next session quyết: commit thành PR riêng hay bỏ. ĐỪNG để lẫn vào commit khác.
