...
| **Đợt 1 — Phòng thi web A1** ✅ code 23/08 (PR xếp chồng lên #377) | UI catalog `/v2/student/speaking/exam` (hệ × cấp × drill/mock, A2–B2 khóa "sắp có") + phòng thi `/session/[id]` (prep → live: stepper, thẻ đề Themen-/Bild-/Keyword-Karte, đồng hồ server, Prüfer/Partner 2 giọng TTS, mic bấm-để-nói, drill có fallback bàn phím, mock chỉ audio → chấm nền poll → Ergebnisbogen theo phiếu từng hệ + khoảng tin cậy + bằng chứng + disclaimer; drill → tổng kết); card ở launcher Speaking; i18n 3 locale; e2e 3 spec; latency mỗi lượt hiển thị ⏱ | **Gate ra mắt public: golden set A1 ≥20 phiên chấm tay đạt ngưỡng đồng thuận (đạt/trượt ≥85%, ±1 band ≥90%)** — chưa làm (cần owner + giáo viên). Còn nợ Đ1: nghiệm thu mic thật trên prod + đo SLO <5s với STT/LLM thật |
| **Đợt 2 — A2** ✅ code 23/08 (PR xếp chồng lên Đ1) | V278: đề A2 (Goethe T1 Stichwort 22 · T2 Von-sich-erzählen 12 · T3 lịch tuần A≠B 10 dùng chung · telc T1 3 · telc T2 Fragewort 24); orchestrator: EXAMINER_LED không đánh vần cho telc A2, hint theo loại thẻ, kịch bản lịch; **đề riêng tư**: `clientStimulus` lược mọi khóa `partner*` (lịch partner chỉ AI biết — nền cho contract B2B); AI partner nhận lịch riêng trong system prompt, Prüfer intro theo loại thẻ; UI thẻ PERSON/QUESTION_WORD/PROMPT/CALENDAR; mở cấp A2; IT mock A2 trọn gói: max 20/25 khi text-only (Aussprache chưa chấm), passRule ngưỡng nói riêng 15 | Còn nợ: Aussprache cho A2 cần audio thật; golden set A2 |
| **Đợt 3 — B1** ✅ code 23/08 (PR xếp chồng lên Đ2) | V279: cột `prep_sec` + đề B1 (Goethe T1 planen 8 · T2 Folien 12 chọn 1/2 · T3 partner trình bày trước 8 bài B1 có lỗi chủ ý · telc T1 Kontaktaufnahme 3 · T2 Vorlage A/B + biểu đồ hư cấu 8 · T3 planen 8). Engine: **Vorbereitungszeit có tài liệu** (`prepMaterials`, client chỉ phần thí sinh), **prep rút gọn 5′ mặc định / FULL theo blueprint**, **chọn 1 trong N** (`POST /choice`, khóa sau khi đã nói — 409), **2 lượt AI sau 1 lượt thí sinh** (T3: partner trả lời + giám khảo hỏi; `aiTurns`), telc T1 kết bằng câu hỏi giám khảo, Vorlage B riêng tư trong system prompt; UI thẻ PLANNING/FOLIEN/PARTNER_PRESENTATION/CONTACT/GRAPHIC (SVG), màn prep có chọn đề, catalog mở B1 + toggle prep. IT mock B1 text-only: max 84/100 (Aussprache 16 chưa chấm). **Đồng thời vá bug có sẵn `AiJobWorker` (tự-gọi `claimJobs` mất REQUIRES_NEW → worker KHÔNG claim được job từ 10/06)** — chặn chấm mock + PRONUNCIATION_EVAL/INTERVIEW_REPORT trên prod; IT hồi quy. | Còn nợ: golden set B1; asset Grafik thật; Aussprache cần audio |
| **Đợt 4 — B2** ✅ code 24/08 (PR #385) | V281 ngân hàng đề B2 (Goethe T1 16 chủ đề Vortrag kèm 3 gạch nội dung + T2 12 câu Debattierclub; telc T1 30 Präsentation + T2 8 text tranh luận + T3 8 Problemlösung). Khảo sát trước khi code cho thấy hạ tầng B2 đã có sẵn từ Đợt 0: blueprint + rubric 2 hệ trong V277, luật Erfüllung=E→Teil 0 trong RubricScorer, ngưỡng telc 45/75, mapper đã có nhánh B2, và orchestrator MONOLOGUE+PRESENT+hasPartner vốn đã cho partner hỏi ngay sau Vortrag rồi mới tới giám khảo (đúng quy chế, không phải sửa). Ba chỗ hụt đã bổ sung: (1) `privateContext` không biết khoá `partnerStance` → partner-AI không giữ vai phản biện; (2) nhánh DIALOGUE mặc định dẫn sang "chốt phương án" kiểu lập kế hoạch → thêm hint riêng cho DEBATE_*; (3) StimulusCard thiếu TOPIC_CHOICE/DEBATE_CARD/DEBATE_TEXT. Mở khoá B2 trong catalog, thêm C1 (khoá) để giữ nhánh "sắp có" có test. | Còn nợ: **golden set B2** (gate ra mắt); **A\* vượt cấp của telc B2** chưa hiện thực (cờ `aStarEqualsA` là cờ CHẾT — không code nào đọc, `BandScales.normalize` strip `*` vô điều kiện, `bandLegend` không nói LLM biết A\* tồn tại); Aussprache B2 cần audio thật (text-only: Goethe trần 84, telc trần 63). |
| **Đợt 5 — Ôn yếu điểm + mobile** — 🟡 5a Ôn yếu điểm ✅ code 24/08 (nhánh `feat/exam-speaking-d5`) | **5a (xong):** cầu nối lỗi → kho SRS: lỗi Ergebnisbogen (mock, lần chấm đầu) + corrections drill (quickEval thêm `severity` MINOR/MAJOR/BLOCKING) đổ vào `user_grammar_errors`+`user_error_skills`+review task qua `GrammarPersistenceService.persistExamError` (sessionId=null CÓ CHỦ Ý — id phiên exam không được nhiễm vào join `findBySessionId` của phiên AI-speaking; không ghi `user_error_observations` vì bảng đòi message_id NOT NULL; mã OTHER bỏ qua); V282 `speaking_exam_error_stats` (user×hệ×cấp×Teil×archetype×mã) upsert lúc ingest làm facet "lọc theo dạng bài"; `GET /api/speaking/exam/weakness` = stats × skill priority × ví dụ mới nhất + gói Redemittel (`examspeaking/redemittel_packs.json`, 10 archetype × 2 dải A1_A2/B1_B2, tự soạn theo chức năng ngôn ngữ); FE `/v2/student/speaking/exam/weakness` (lọc chip archetype, ví dụ sai→đúng, CTA tạo DRILL đúng Teil từ ngữ cảnh) + card lối vào catalog + i18n 3 locale. Test: unit 13 + IT 2 (drill → đủ 4 bảng + endpoint; empty-state vẫn có packs) + e2e 3 spec (bẫy: `waitForURL` phải regex chấp nhận trailing slash). **Còn 5b:** parity mobile Expo (doc B2B §11) + admin dashboard đề | Còn nợ 5a: nghiệm thu prod sau deploy (cần token); 5b chưa bắt đầu |
...
| **AiJobWorker không claim job (bug có sẵn từ a7e48b28 10/06, phát hiện 23/08)** — tự-gọi `claimJobs()` nên `@Transactional(REQUIRES_NEW)` vô hiệu → `bulkUpdateStatus` ném TransactionRequiredException mỗi 2s | ✅ vá ở PR Đ3 bằng `TransactionTemplate` REQUIRES_NEW tường minh + IT hồi quy `AiJobWorkerClaimIntegrationTest`; kiểm lại job PRONUNCIATION_EVAL/INTERVIEW_REPORT trên prod sau deploy |
