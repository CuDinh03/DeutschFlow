...
Ngày: 2026-07-14 · Trạng thái: **C ✅ commit `65d03f68` + push · A (FE icon vocab) ✅ code+test** · Scope: mobile + backend

> **A — Tầng 1 client-side ✅ (14/07):** `mobile/lib/vocabGlyph.ts` (thuần) map từ Đức → icon Lucide + tint. ⚠️ **Mạo từ der/die/das VẪN HIỆN đầy đủ ở mọi màn** (yêu cầu USER 14/07 — rất quan trọng). Việc "strip mạo từ" CHỈ nằm trong `normalizeGerman()` để TRA icon ("der Apfel"→"apfel"→🍎), KHÔNG đụng chữ hiển thị. Exact-match dict ~140 từ; fallback keyword nghĩa tiếng Việt; **trả `null` cho từ trừu tượng** (không icon, đúng D1 Tầng 3). Render qua `components/ui/VocabGlyphTile.tsx` ở **3 chỗ**: flashcard SRS (`srs.tsx`, tile 96px trên từ — mạo từ giữ nguyên trước từ), từ điển (`vocabulary.tsx` WordRow — slot icon 40px + mạo từ inline tô màu theo mockup), lesson vocab (`node.tsx` VocabRow, tile 36px — mạo từ là Pill giữ nguyên). JS-only → **OTA-safe, KHÔNG cần `expo-image`/build 13**. `tsc` 0 lỗi, jest **228/228** (+18). **Review đối kháng ✅ đã vá:** HIGH `VI 'tiền'` bắt nhầm "tiền tố"/thẻ chữ cái (verify trên seed prod V247/V127) → đổi cụm cụ thể + regression test; MEDIUM căn lề node/vocabulary → slot icon cố định + **khôi phục CỘT mạo từ riêng ở WordRow** (mạo từ tô màu, đúng vị trí — theo yêu cầu USER); bỏ `Tee`→cà phê / `Saft`→nước (icon trùng/sai). **Backend icon_key + B0 (LLM map) = follow-up** — render layer đã sẵn nhận `icon_key` khi có.
