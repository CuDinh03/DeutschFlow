...
| P3 | Calibration (F1) + flip model | 🔄 **F2.4 XONG** — script contract-test vào repo, 8/8 tier đạt trên Fireworks thật. 🔴 **Đo lòi ra: CẢ 3 ứng viên F1 không chạy nổi ở ngân sách token hiện tại** ⇒ flip KHÔNG còn là "sửa 1 dòng env"; phát sinh F1.0 + tiền đề F3.4. F1.1–F1.5 còn nguyên. Số đo: `BAO_CAO_CONTRACT_TEST_TIER_2026-08-09.md` | [#313](https://github.com/CuDinh03/DeutschFlow/pull/313) | — |
...
**Còn mở:**
- [ ] 👤 FW.1 Deploy `./deploy-backend.sh` — gom cả #309 + #310 + #311 và 2 env tier ở FW.c. Soi SHA `[2/6] Pull code` = **`4c34c29c`** (không còn là `346959f4`; worktree deploy nhớ `git merge --ff-only origin/main` trước, xem sự cố push non-fast-forward 09/08).
...
- [ ] F1.1 Mở rộng `/api/admin/grading-eval`: nhận tham số tier/model list, chấm N bài đã lưu bằng từng model, xuất JSON/CSV (điểm, band, thời gian, cost). ⚠️ **phải đo cả "mất nhận xét"** (bài học FW.7: điểm vẫn có nhờ regex fallback nhưng feedback rỗng) và cho đặt max_tokens theo lượt đo (F1.0).
