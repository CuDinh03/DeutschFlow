...
- [ ] 🔎 **B1.9 / B1.10 (phát hiện khi làm F3.4b)** — tier `CONTENT` đang gánh 2 việc KHÔNG phải sinh nội dung: `SkillTreeService:1107` là **tips phát âm** (`PRONUNCIATION_EVAL`) và `:1248` là **báo cáo phỏng vấn** (`INTERVIEW_REPORT`) — đánh giá per-user, real-time, đúng họ với 6 mis-route đã sửa ở B1.1–B1.8. Đây cũng chính là 2 chỗ ngân sách 1024 token làm K2.6 trả rỗng. Đề nghị route về `GRADING_DAILY`; **chưa làm vì đổi model đang phục vụ người dùng cần quyết định riêng.**
