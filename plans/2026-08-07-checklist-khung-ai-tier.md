...
- [x] ✅ **F3.4b XONG (thay F3.4, quyết định #15)** — [PR #318](https://github.com/CuDinh03/DeutschFlow/pull/318), CI xanh gồm IT. Tầng **`CONTENT_BATCH`** mới (default gương `content` ⇒ zero behavior change) + `SkillTreeContentPregenerationService` + `POST /api/admin/skill-tree/pregenerate` (ADMIN, body rỗng ⇒ dryRun). Chỉ LẤP node rỗng — `UPDATE … WHERE content_json IS NULL` nên chạy lại an toàn/tự resume, không đụng nội dung học viên đang học; AI trả rỗng ⇒ FAILED chứ không ghi bài học trắng; một node lỗi không sập cả lô; tuần tự, trần 25 node/lần. Test 9/0, full unit 1815/0.
  - 👤 **Flip khi muốn dùng K2.6:** `AI_LLM_TIER_CONTENT_BATCH_MODEL=accounts/fireworks/models/kimi-k2p6` + `AI_LLM_TIER_CONTENT_BATCH_EFFORT=` (RỖNG — K2.6 không thuộc họ gpt-oss, bài học FW.7). Chạy `dryRun=true` trước để xem đúng node nào bị đụng.
- [x] ✅ **B1.9 / B1.10 XONG** — [PR #319](https://github.com/CuDinh03/DeutschFlow/pull/319), CI xanh. **(phát hiện khi làm F3.4b)** — tier `CONTENT` đang gánh 2 việc KHÔNG phải sinh nội dung: `SkillTreeService:1107` là **tips phát âm** (`PRONUNCIATION_EVAL`) và `:1248` là **báo cáo phỏng vấn** (`INTERVIEW_REPORT`) — đánh giá per-user, real-time, đúng họ với 6 mis-route đã sửa ở B1.1–B1.8. Đây cũng chính là 2 chỗ ngân sách 1024 token làm K2.6 trả rỗng. Đã route về `GRADING_DAILY`. **Đo trước khi flip bằng prompt THẬT** (Fireworks, 120b + effort=low, giữ nguyên 1024 tok, 6 lượt/luồng): INTERVIEW_REPORT 6/6 JSON hợp lệ out 248–313 (biên 69%) · PRONUNCIATION_EVAL 6/6 out 145–445 (biên 56%) ⇒ không cần nới max_tokens. Đánh đổi: 20b→120b, latency +~1s, đơn giá ~2× trên lượng token nhỏ; cả hai không nằm trong đường SSE real-time. Test `SkillTreeEvalTierTest` 3/0 (trước đó 2 luồng KHÔNG có test nào), full unit 1809/0. 🔑 Một phần của "K2.6 làm hỏng cây học tập" hoá ra là do mis-route này chứ không chỉ do ngân sách.
