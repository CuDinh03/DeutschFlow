...
2. **P1 (SVG pilot, 0đ API)** ✅ (16/08): 30 SVG v2 sinh trong phiên Claude Code, validator PASS 30/30, owner duyệt style (chốt model `claude-fable-5` qua A/B); 3 quy tắc art-direction bổ sung nạp vào prompt (mặt biểu cảm / tính từ đối chứng / động từ micro-scene). Anchors chính thức: `01-der-apfel`, `16-lesen`, `24-gross`.
3. **P2** ✅ code xong (16/08, nhánh `worktree-galerie-p2`): `GalerieSvgSanitizer` (whitelist element/attribute/5-hex, cap 20KB, jsoup) + `GaleriePromptFactory.svgPrompt` (system+anchors resources, cache_control) + `GalerieAnthropicClient` (Fable 5: không gửi thinking, xử lý refusal, fallbacks "default" beta 2026-07-01, lazy init) + `GalerieSvgGenerationService` (claim GENERATING→QA_PENDING, S3 `galerie/galerie-v1/{wordId}.svg`, ledger `anthropic/galerie.svg`) + endpoints `POST /generate`, `GET /generate/ready-count`, `POST /{wordId}/decision` + tab Galerie trang admin vocabulary (grid + 3 nút). **Sync-only** — Batch API 50% giá dời sang P4. **Đường 0đ (chốt 17/08): `POST …/galerie/{wordId}/artwork` nhận SVG vẽ SẴN** — tranh sinh ngay trong phiên Claude Code (gói Max của owner = Fable 5, không tốn API) đẩy lên prod qua endpoint này (qua đúng sanitizer + luồng lưu, ra QA_PENDING chờ duyệt; không cần `ANTHROPIC_API_KEY`). Quy trình: merge+deploy → owner đưa token ADMIN (TTL 15') → session map từ↔id qua `GET /vocabulary/words` → POST từng SVG. `ANTHROPIC_API_KEY` chỉ còn cần khi muốn backend tự vẽ (nút bấm/Batch API P4) — dời thành tùy chọn.
