...
| Data controller (legal) | `Dinh Huy Cu` / `Đinh Huy Cự` (deploy `41f6da00`) |
...
| `[[support-contact-name]]` | Tên liên hệ (vd "Dinh Huy Cu") | — |
...
### 2.4 ✅ Tên seller hợp pháp (`LEGAL_NAME`) — ĐÃ CHỐT 2026-07-03

**Đã xong.** Chủ app đổi `LEGAL_NAME_EN = 'Dinh Huy Cu'` / `LEGAL_NAME_VI = 'Đinh Huy Cự'` trong `frontend/scripts/gen-legal.mjs`, đã **regenerate** `privacy.ts` + `terms.ts` (`node scripts/gen-legal.mjs`) và **push → Amplify redeploy** (commit `41f6da00`). Live `/privacy` giờ ghi **"operated by Dinh Huy Cu"**; free-only nguyên vẹn (0 AdMob).

> ⚠️ **Việc còn lại (1 tick khi vào ASC):** đối chiếu tên này **khớp CHÍNH XÁC** tên Apple hiển thị là *seller* (ASC → Business/Agreements) + tên trên CCCD. Nếu Apple hiển thị dạng khác → sửa lại `LEGAL_NAME_EN/VI` trong `gen-legal.mjs` → `node scripts/gen-legal.mjs` → commit → redeploy. v1.0 free-only chưa ràng W-8BEN/thuế, nhưng khớp tên ngay tránh sửa legal lại ở v1.1.
...
- Tên seller: `frontend/scripts/gen-legal.mjs` = `'Dinh Huy Cu'`/`'Đinh Huy Cự'` — đã regenerate + deploy (`41f6da00`).
...
2. **App Review Information** *(theo `STORE_COPY.md`)*:
   ...
   | **Contact** | `Dinh Huy Cu` · `dinhhuycu0305@gmail.com` · `[[phone]]` |
...
- Data controller privacy = **"Dinh Huy Cu"** — dùng đồng nhất tên/email liên hệ này nếu khai Trader.
