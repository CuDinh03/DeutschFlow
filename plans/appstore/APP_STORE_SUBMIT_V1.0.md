...
| Data controller (legal) | `Cu Dinh` / `Cự Đinh` — khớp ASC Legal Entity (re-set 2026-07-04) |
...
| 2.4 | ✅ **Tên seller** `LEGAL_NAME` = "Cu Dinh"/"Cự Đinh" — khớp ASC Legal Entity; re-set + regenerate 2026-07-04 (commit local `053ea2df`, **chờ push → Amplify redeploy**) | 📄✅ + 🌐 tick | 🟡 | ☑ |
...
### 2.4 ✅ Tên seller hợp pháp (`LEGAL_NAME`) — ĐÃ CHỐT "Cu Dinh" (2026-07-04)

**Bối cảnh.** Deploy cũ `41f6da00` (2026-07-03) đặt tạm `LEGAL_NAME` = `'Dinh Huy Cu'`/`'Đinh Huy Cự'`. **2026-07-04** khi vào ASC → **Business → Edit Legal Entity**, tên Apple đăng ký là **"Cu Dinh"** (Individual, Hà Nội). Đã **re-set** `LEGAL_NAME_EN='Cu Dinh'` / `LEGAL_NAME_VI='Cự Đinh'` trong `frontend/scripts/gen-legal.mjs`, **regenerate** `privacy.ts` + `terms.ts` (`node scripts/gen-legal.mjs`) → verify sạch (`operated by Cu Dinh` / `do Cự Đinh vận hành`, 0 vết tên cũ). Commit local **`053ea2df`**.

> ⚠️ **Còn 1 thao tác để LÊN LIVE:** `git push origin main` (kích Amplify ~3'). Guardrail chặn push tự động → **chủ app tự chạy push** hoặc cho phép. Sau redeploy, verify `curl -sL https://mydeutschflow.com/privacy | grep "Cu Dinh"`.
>
> 📌 **CCCD/thuế (v1.1):** "Cu Dinh" khớp **seller name Apple hiển thị** → đúng yêu cầu Guideline 5.1.1 cho v1.0 free-only. Tên đầy đủ trên CCCD chỉ bắt buộc khớp khi ký **Paid Apps Agreement + W-8BEN** (khi thêm IAP ở v1.1) — lúc đó rà lại nếu Apple đổi seller name.
...
- Tên seller: `frontend/scripts/gen-legal.mjs` = `'Cu Dinh'`/`'Cự Đinh'` — khớp ASC Legal Entity; re-set + regenerate 2026-07-04 (commit local `053ea2df`, chờ push → Amplify redeploy).
