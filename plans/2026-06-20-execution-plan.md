...
| 2026-06-21 | **QA Prototype A** | ✅ Đối chiếu `~/Downloads/deutschflow/Prototype A - Galerie.html` + `proto-org-admin.jsx`. **Phát hiện:** admin-12 KHÔNG có trong proto (→ chỉ design-system compliance); org-class/student-detail CÓ proto nhưng dựa mock progress/payment/schedule không-backend → **Option-1**. Nâng `org-class-detail` = 2-cột + side panel (GV + điểm-KN-TB thật) + token-discipline `var(--ga-*)` (cả 2 org-detail). tsc+eslint sạch. **Doc:** `docs/UI_2.0_QA_W1.7_PROTOTYPE_A_2026-06-21.md` (gồm backend-gaps để full-fidelity). Visual runtime-QA ⏳ W1.6. |
