...
/** Base URL for raw fetch with SSE */
const API_BASE = (process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8080') + '/api'


/** Groq queue + long JSON streams need a longer idle window than local model. */
const STALL_MS =
  Number(process.env.NEXT_PUBLIC_AI_SPEAKING_STREAM_STALL_MS) ||
  (process.env.NEXT_PUBLIC_AI_CHAT_PROVIDER === 'groq' ? 90_000 : 45_000)
