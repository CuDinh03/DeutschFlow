...
export function scoreAttempt(
  ...
  const normalizedAttempt = normalizeWord(rawAttempt)
  const missingAnchors: string[] = []
  const orderViolations: string[] = []
  if (!rule) {
    const ratio =
      targetGerman && normalizedAttempt.length > 0
        ? levenshtein(normalizedAttempt, normalizeWord(targetGerman)) /
          Math.max(normalizedAttempt.length, normalizeWord(targetGerman).length, 1)
        : 1
    return {
      pass: ratio <= 0.35,
      normalizedAttempt,
  ...
  let ratio = 0
  if (targetGerman) {
    const nt = normalizeWord(targetGerman)
    ratio =
      nt.length > 0
