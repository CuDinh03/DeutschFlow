...
export default function ListeningView({ content, isLocked = false }: { content: NodeContent; isLocked?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Audio Player ── */}
      <div className="rounded-ga bg-gradient-to-r from-ga-ink to-ga-ink p-5 space-y-4">
        <div className="flex items-center gap-2">
          <Volume2 size={16} className="text-ga-yellow" />
          <span className="text-sm font-bold text-white">Bài nghe</span>
          <button
            type="button"
            onClick={changeSpeed}
            className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-ga-card/10 text-white/80 hover:bg-ga-card/20 transition-colors font-mono"
          >
            {speed}x
          </button>
        </div>
        ...
        {/* Progress bar */}
        <div
          className="relative h-2 bg-ga-card/10 rounded-full cursor-pointer group"
          onClick={(e) => {
        ...
        >
          <div
            className="absolute h-full bg-ga-yellow rounded-full transition-[background-color,border-color,color,box-shadow,transform,max-height,width] duration-100"
            style={{ width: `${duration ? (currentTime / duration) * 100 : 0}%` }}
          />
          <div
            className="absolute w-3 h-3 bg-ga-card rounded-full -top-0.5 shadow-ga-card-hover opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={restart} className="w-8 h-8 rounded-full bg-ga-card/10 flex items-center justify-center text-white/70 hover:bg-ga-card/20 transition-colors">
              <RotateCcw size={14} />
            </button>
            <button
              type="button"
              onClick={togglePlay}
              className="w-12 h-12 rounded-full bg-ga-yellow flex items-center justify-center text-ga-ink hover:scale-105 active:scale-95 transition-transform shadow-ga-card-hover"
            >
      ...
      {/* ── Karaoke Transcript ── */}
      {timestamps.length > 0 && (
        <div ref={transcriptRef} className="rounded-ga bg-ga-card border border-ga-line p-4 max-h-[300px] overflow-y-auto">
          ...
          <div className="flex flex-wrap gap-1 leading-loose">
            {timestamps.map((w, i) => {
              ...
              if (isBlank) {
                ...
                let inputClass = "border-ga-subtle bg-transparent text-ga-muted";
                if (isLocked || submitted) {
                  inputClass = isCorrect
                    ? "border-ga-green bg-ga-green-soft text-ga-green"
                    : "border-ga-red/50 bg-ga-red-soft text-ga-red";
                } else if (isActive) {
                  inputClass = "border-ga-gold bg-ga-yellow-soft";
                }
              ...
              return (
                <span
                  ...
                  className={`cursor-pointer px-0.5 rounded transition-[background-color,border-color,color,box-shadow,transform,max-height,width] duration-150 text-sm ${
                    isActive ? "bg-ga-yellow text-ga-ink font-bold scale-105" :
                    isPast ? "text-ga-subtle" :
                    "text-ga-ink hover:bg-ga-surface"
                  }`}
                >
                  {w.word}
                </span>
      ...
      {timestamps.length > 0 && !submitted && !isLocked && blankIndices.size > 0 && (
        <button
          ...
          disabled={!allFilled}
          className={`w-full py-3 rounded-ga font-bold text-sm transition-[background-color,border-color,color,box-shadow,transform,max-height,width] duration-200 flex items-center justify-center gap-2 ${
            allFilled
              ? "bg-ga-ink text-white hover:bg-ga-ink shadow-ga-card-hover"
              : "bg-ga-line text-ga-subtle cursor-not-allowed"
          }`}
        >
          <CheckCircle size={16} />
      ...
      {submitted && score !== null && (
        <div className={`rounded-ga p-5 text-center space-y-2 border-2 ${
          score >= 100 ? "bg-ga-green-soft border-ga-green/50" :
          score >= 80  ? "bg-ga-yellow-soft border-ga-gold/50" :
          "bg-ga-red-soft border-ga-red/50"
        }`}>
      ...
      {isCompleted && passageText && (
        <div className="rounded-ga border border-ga-violet/30 bg-ga-violet-soft overflow-hidden">
          <button
            type="button"
            onClick={() => setShowShadowing(v => !v)}
            className="w-full flex items-center justify-between px-4 py-3 text-sm font-semibold text-ga-violet hover:bg-ga-violet-soft transition-colors"
          >
