...
import { AiMessageBubble, Exchange, CYAN, PURPLE, MINT, SPEAKING_LIGHT, SPEAKING_DARK, glassLight, glass } from "./types";
...
export function SessionSummary({
  ...
  const T = variant === "dark" ? SPEAKING_DARK : SPEAKING_LIGHT;
  const cardStyle = variant === "dark" ? glass : glassLight;
  ...
  return (
    <motion.div className="flex flex-col gap-5 pb-6"
      ...
      <div className="text-center pt-2">
        ...
        <h2 className="font-extrabold text-xl" style={{ color: T.ink }}>{t("summaryTitle")}</h2>
        <p className="text-sm mt-1" style={{ color: T.inkSoft }}>{t("summarySubtitle")}</p>
      </div>
      ...
      <div className="rounded-[20px] p-5" style={cardStyle}>
        <div className="flex items-center gap-5">
          <div className="relative flex-shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-extrabold text-3xl leading-none" style={{ color: T.ink }}>{animScore}</span>
        ...
        <div className="mt-4">
          <div className="flex justify-between mb-1.5">
            <span className="text-xs" style={{ color: T.inkSoft }}>{t("summaryFluency")}</span>
          ...
          <div className={isDark ? "h-2 rounded-full overflow-hidden bg-white/10" : "h-2 rounded-full overflow-hidden bg-slate-200"}>
      ...
      {learnedVocab.length > 0 && (
        <div className="rounded-[20px] overflow-hidden" style={cardStyle}>
          <div
            className={
              isDark
                ? "flex items-center gap-2 px-4 py-3 border-b border-white/10"
                : "flex items-center gap-2 px-4 py-3 border-b border-slate-200/90"
            }
          >
            <BookOpen size={14} style={{ color: CYAN }} />
            <span className="font-semibold text-sm" style={{ color: T.ink }}>{t("summaryNewVocab")}</span>
      ...
      {commonMistakes.length > 0 && (
        <div className="rounded-[20px] overflow-hidden" style={cardStyle}>
          ...
          >
            ...
            <span className="font-semibold text-sm" style={{ color: T.ink }}>{t("summaryCommonMistakes")}</span>
          </div>
            ...
            return (
              <motion.div key={m.key} className="flex items-start gap-3 px-4 py-3"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold" style={{ color: T.ink }}>{m.title}</p>
                  <p className="text-xs mt-0.5 break-words" style={{ color: T.inkSoft }}>{m.detail}</p>
                  {m.errorCode && i < 3 && (
                    <button
                      type="button"
                      onClick={() => {
