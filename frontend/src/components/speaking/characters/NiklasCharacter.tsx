...
export type NiklasExpression = "neutral" | "talking" | "smiling" | "thinking" | "serious";

interface Props {
  expression?: NiklasExpression;
  isTalking?: boolean;
  className?: string;
  style?: CSSProperties;
}

function Eyebrows({ expression }: { expression: NiklasExpression }) {
...
function Eyes({ blinking }: { blinking: boolean }) {
  ...
  return (
    <g>
      ...
      <ellipse cx="126" cy="177" rx="6" ry="6" fill="#1565C0" /> {/* Blue eyes */}
...
export function NiklasCharacter({ expression = "neutral", isTalking = false, style, className }: Props) {
