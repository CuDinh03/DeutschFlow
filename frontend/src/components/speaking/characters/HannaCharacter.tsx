...
export type HannaExpression = "neutral" | "talking" | "smiling" | "thinking" | "serious";

interface Props {
  expression?: HannaExpression;
  isTalking?: boolean;
  className?: string;
  style?: CSSProperties;
}
...
// ── Eyebrows ──────────────────────────────────────────────────────
function Eyebrows({ expression }: { expression: HannaExpression }) {
  const c = "#134e4a";
  const sw = 5.5;
  switch (expression) {
    case "smiling":
...
// ── Eyes ──────────────────────────────────────────────────────────
function Eyes({ blinking, expression }: { blinking: boolean; expression: HannaExpression }) {
...
function MouthFrame({ frame, expression }: { frame: number; expression: HannaExpression }) {
...
// ── Main Component ────────────────────────────────────────────────
export function HannaCharacter({ expression = "neutral", isTalking = false, style, className }: Props) {
  ...
  return (
    ...
    >
      ...
      />
      {/* Sleeve cuff hints */}
      <path d="M 28 422 C 36 442 66 446 80 437" fill="none" stroke="#115e59" strokeWidth="2" opacity="0.45" strokeLinecap="round" />
      <path d="M 260 422 C 252 442 222 446 208 437" fill="none" stroke="#115e59" strokeWidth="2" opacity="0.45" strokeLinecap="round" />

      {/* ── Eco motif on chest ── */}
      <text x="100" y="344" fontSize="8" fill="#5eead4" fontFamily="monospace" letterSpacing="3" opacity="0.9">♻︎ eco</text>
      <text x="100" y="358" fontSize="8" fill="#5eead4" fontFamily="monospace" letterSpacing="3" opacity="0.85">🌱 bio</text>
      <text x="100" y="372" fontSize="8" fill="#5eead4" fontFamily="monospace" letterSpacing="3" opacity="0.8">♻ ♻ ♻</text>

      {/* ── Leaf badge ── */}
      <circle cx="202" cy="348" r="17" fill="rgba(16,185,129,0.18)" />
      <text x="195" y="353" fontSize="13" opacity="0.75">🌿</text>

      {/* Pocket */}
      <path d="M 108,396 C 108,390 112,386 118,386 L 170,386 C 176,386 180,390 180,396 L 180,432 L 108,432 Z" fill="#115e59" opacity="0.55" />
      <line x1="144" y1="386" x2="144" y2="432" stroke="#0f766e" strokeWidth="1.5" opacity="0.45" />
      ...
      <path d="M 110 264 C 116 259 127 255 144 255 C 161 255 172 259 178 264"
        fill="none" stroke="#115e59" strokeWidth="3" strokeLinecap="round" opacity="0.55" />
      ...
      {/* ── GLASSES ── */}
      <rect x="104" y="167" width="38" height="22" rx="7" fill="rgba(45,212,191,0.16)" stroke={HAIR} strokeWidth="2.8" />
      <rect x="146" y="167" width="38" height="22" rx="7" fill="rgba(45,212,191,0.16)" stroke={HAIR} strokeWidth="2.8" />
