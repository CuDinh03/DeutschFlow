...
const SPEAKING_PERSONAS: ReadonlyArray<{
  ...
  // ── Tutor mặc định ──
  { id: "DEFAULT", emoji: "🎓", nameKey: "personaNameDefault", descKey: "personaDescDefault", group: "default" },
  // ── Core 4 ──
  ...
  { id: "THOMAS",  emoji: "🥐", nameKey: "personaNameThomas",  descKey: "personaDescThomas",  group: "verkauf" },
  { id: "PETRA",   emoji: "🥩", nameKey: "personaNamePetra",   descKey: "personaDescPetra",   group: "verkauf" },
  // ── Medizin (Y khoa) ──
  { id: "SARAH",     emoji: "🏥", nameKey: "personaNameSarah",     descKey: "personaDescSarah",     group: "medizin" },
  { id: "SCHNEIDER", emoji: "👁️", nameKey: "personaNameSchneider", descKey: "personaDescSchneider", group: "medizin" },
  { id: "WEBER",     emoji: "🔬", nameKey: "personaNameWeber",     descKey: "personaDescWeber",     group: "medizin" },
  // ── Maschinenbau (Cơ khí) ──
  { id: "MAX",    emoji: "⚙️", nameKey: "personaNameMax",    descKey: "personaDescMax",    group: "maschine" },
  { id: "OLIVER", emoji: "🔧", nameKey: "personaNameOliver", descKey: "personaDescOliver", group: "maschine" },
  // ── Service (Phục vụ) ──
  { id: "NIKLAS", emoji: "🍷", nameKey: "personaNameNiklas", descKey: "personaDescNiklas", group: "service" },
  { id: "NINA",   emoji: "🏨", nameKey: "personaNameNina",   descKey: "personaDescNina",   group: "service" },
  // ── Medien / MC (Truyền thông) ──
  { id: "HANNIE", emoji: "🎤", nameKey: "personaNameHannie", descKey: "personaDescHannie", group: "medien" },
  // ── Gia sư Việt (LESSON mode) ──
  { id: "TUAN", emoji: "🇻🇳", nameKey: "personaNameTuan", descKey: "personaDescTuan", group: "viet" },
  { id: "LAN",  emoji: "🌸", nameKey: "personaNameLan",  descKey: "personaDescLan",  group: "viet" },
  { id: "MINH", emoji: "🛵", nameKey: "personaNameMinh", descKey: "personaDescMinh", group: "viet" },
];
      ...
      {wizardStep === 1 ? (
        <>
          ...
          <div className="rounded-[20px] p-4" style={glassLight}>
            ...
            {/* Render theo nhóm */}
            {Object.entries(
              SPEAKING_PERSONAS.reduce<Record<string, typeof SPEAKING_PERSONAS>>((acc, p) => {
              ...
              <div key={group} className="mb-3 last:mb-0">
                <p className="mb-1.5 text-[9px] font-bold uppercase tracking-widest" style={{ color: SPEAKING_LIGHT.inkFaint }}>
                  {PERSONA_GROUP_LABELS[group] ?? group}
                </p>
                <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
                  {personas.map(({ id, emoji, nameKey, descKey }) => (
                    <button
                      key={id}
                      type="button"
                      onClick={() => setPersona(id)}
                      className="flex flex-col items-start gap-0.5 rounded-[12px] p-2.5 text-left transition-all"
                      style={{
                    ...
                    >
                      ...
                      <span className="text-[8px] leading-snug" style={{ color: SPEAKING_LIGHT.inkFaint }}>
                        {t(descKey)}
                      </span>
                    </button>
                  ))}
        ...
        <>
          ...
          {quotaBlocked && (
            <SpeakingQuotaBlockedBanner className="mb-4" compact />
          )}

          <motion.button
            type="button"
            onClick={() => onStart(custom.trim() || selected || undefined, cefrLevel, persona, responseSchema, learningMode)}
            disabled={isStarting || quotaBlocked || quotaLoading}
