...
import type { SpeakingPersonaId, SpeakingResponseSchemaId } from "@/lib/aiSpeakingApi";
...
export function WelcomeScreen({
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-5 flex flex-col gap-4"
      ...
      {planCurrentLevel || planTargetLevel ? (
        <p className="text-[11px] leading-snug px-1 rounded-[10px] py-2 bg-slate-100/90" style={{ color: SPEAKING_LIGHT.inkMuted }}>
          {t("levelRangeHint", { floor: floorBand, ceil: ceilBand })}
        </p>
      ) : null}

      {/* Hero */}
      <div className="rounded-[20px] p-5 relative overflow-hidden border border-cyan-200/80 bg-white/80 shadow-sm"
        ...
        <div className="absolute -top-6 -right-6 w-32 h-32 rounded-full"
          style={{ background: `radial-gradient(circle, rgba(34,211,238,0.18) 0%, transparent 70%)`, filter: "blur(20px)" }} />
        <div className="relative z-10">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold mb-3"
            style={{ background: `${CYAN}18`, border: `1px solid ${CYAN}44`, color: "#0e7490" }}>
            ✨ {t("welcomeBadge")}
          </div>
          <h2 className="font-extrabold text-xl mb-1" style={{ color: SPEAKING_LIGHT.ink }}>{t("welcomeTitle")}</h2>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {[t("tagInstantCorrection"), t("tagGrammarTips"), t("tagNaturalChat")].map((label) => (
              <span key={label} className="text-[11px] px-2.5 py-1 rounded-full bg-white/85 border border-slate-200/90"
      ...
      <div className="rounded-[20px] p-4" style={glassLight}>
        <p className="text-xs font-semibold mb-3" style={{ color: SPEAKING_LIGHT.inkSoft }}>{t("chooseLevel")}</p>
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
          {CEFR_LEVELS.map(({ id, emoji, color, desc }) => {
            ...
            <button
              ...
              <span className="text-xs font-extrabold" style={{ color: cefrLevel === id ? color : SPEAKING_LIGHT.ink }}>{id}</span>
              <span className="text-[9px] leading-tight text-center" style={{ color: SPEAKING_LIGHT.inkFaint }}>{t(desc)}</span>
            </button>
            );
          })}
        </div>
      ...
      {/* Response contract V1 / V2 (parallel) */}
      <div className="rounded-[20px] p-4" style={glassLight}>
        <p className="text-xs font-semibold mb-3" style={{ color: SPEAKING_LIGHT.inkSoft }}>{t("chooseResponseSchema")}</p>
        <div className="grid grid-cols-2 gap-2">
          {(
            ...
            <button
              ...
              className="flex flex-col items-start gap-1 p-3 rounded-[14px] transition-all text-left"
              style={{
            ...
            >
              <span className="text-[11px] font-semibold" style={{ color: responseSchema === id ? "#0e7490" : SPEAKING_LIGHT.ink }}>
                {t(nameKey)}
              </span>
              <span className="text-[9px] leading-snug" style={{ color: SPEAKING_LIGHT.inkFaint }}>{t(descKey)}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Persona */}
      <div className="rounded-[20px] p-4" style={glassLight}>
        <p className="text-xs font-semibold mb-3" style={{ color: SPEAKING_LIGHT.inkSoft }}>{t("choosePersona")}</p>
          ...
          {SPEAKING_PERSONAS.map(({ id, emoji, nameKey, descKey }) => (
            <button
              ...
              style={{
                ...
                border: persona === id ? `1px solid ${PURPLE}55` : `1px solid ${SPEAKING_LIGHT.line}`,
              }}
            >
              <span className="text-lg">{emoji}</span>
              <span className="text-[11px] font-semibold leading-tight" style={{ color: persona === id ? PURPLE : SPEAKING_LIGHT.ink }}>
                {t(nameKey)}
              </span>
      ...
      {/* Topic grid */}
      <div className="rounded-[20px] p-4" style={glassLight}>
        <p className="text-xs font-semibold mb-3" style={{ color: SPEAKING_LIGHT.inkSoft }}>{t("chooseTopic")}</p>
        <div className="grid grid-cols-3 gap-2 mb-4">
          {SUGGESTED_TOPICS.map(({ id, emoji, labelKey, descKey }) => (
            <button key={id} onClick={() => { setSelected(id); setCustom(""); }}
              ...
              <span className="text-[11px] font-semibold" style={{ color: selected === id && !custom ? "#0e7490" : SPEAKING_LIGHT.ink }}>{t(labelKey)}</span>
              <span className="text-[9px]" style={{ color: SPEAKING_LIGHT.inkFaint }}>{t(descKey)}</span>
            </button>
          ))}
        </div>
        <div className="relative">
          <input type="text" value={custom}
            ...
            className="w-full px-4 py-2.5 pr-8 rounded-[12px] text-sm outline-none bg-white border"
            style={{
      ...
      {/* Start button */}
      <motion.button
        onClick={() => onStart(custom.trim() || selected || undefined, cefrLevel, persona, responseSchema)}
        disabled={isStarting}
