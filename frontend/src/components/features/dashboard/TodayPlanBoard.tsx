    ...
    <div className="w-full">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="font-bold text-blue-600 dark:text-blue-400">{plan.streakDays ?? 0}%</span>
        </div>
        <div className="h-3 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
          <motion.div 
            initial={{ width: 0 }}
            animate={{ width: `${Math.min(100, Math.max(0, ((plan.suggestedLessons?.length ?? 0) + (plan.errorReviewList?.length ?? 0)) * 20))}%` }}
