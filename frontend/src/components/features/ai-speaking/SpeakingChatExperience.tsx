...
// page `app/speaking/chat/page.tsx`; it is a component now so the /v2 (Galerie) surface can
// mount the SAME engine instead of deep-linking back into v1. Nothing here changed except
// that routes are injected (`routes`) and the root height adapts to the host shell (`layout`).

type ViewMode = "chat" | "summary";

/** Exit routes of the flow — differ per shell (legacy pages vs /v2/student/*). */
export interface SpeakingChatRoutes {
  /** Companion picker: guard when no companion in store, header back, summary "restart". */
  setup: string;
  /** SRS review after importing the session errors. */
  review: string;
  /** Past sessions. `null` = no equivalent surface on this shell → the button is hidden. */
  history: string | null;
  /** Fallback exit target when the store carries no `returnPath`. */
  home: string;
  /** Upgrade surface behind the quota-blocked banner (must stay inside the caller's shell). */
  pricing: string;
}

export interface SpeakingChatExperienceProps {
  routes: SpeakingChatRoutes;
  /**
   * "page"  — legacy full-viewport route (own scroll context, h-screen).
   * "shell" — embedded in the /v2 GaShell `<main>`, which already has a bounded height,
   *           so the engine must fill it (h-full) instead of adding a second viewport.
   */
  layout?: "page" | "shell";
}
...
export function SpeakingChatExperience({ routes, layout = "page" }: SpeakingChatExperienceProps) {
  ...
  }, [selectedCompanion, router, routes.setup, sessionMode, trackFeatureAction]);
  ...
  // Bridge (wave 1): a session STARTED from a /v2 surface must land back in /v2 even while the
  // engine still runs inside the legacy shell. Drop this branch once v1 is deleted.
  const reviewPath = returnPath?.startsWith("/v2/student") ? "/v2/student/review" : routes.review;
          ...
          <SessionSummary
            ...
            onViewHistory={
              routes.history
                ? () => {
                    clearChat();
                    router.push(routes.history as string);
                  }
                : undefined
            }
            onRestart={() => {
            ...
            onExit={() => {
              clearChat();
              router.push(returnPath || routes.home);
            }}
          />
        </div>
      </div>
