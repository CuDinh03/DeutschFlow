...
import type { AccountSource } from './machine'

/**
 * `GET /onboarding/context` — Đợt 5 kế hoạch onboarding 17/09/2026 (§4.1 "Ba cửa vào").
 *
 * Một nguồn duy nhất để web rẽ lối: học viên trung tâm (`ORG_ROSTER` | `ORG_INVITE`) chưa có plan đi
 * bản rút gọn `PROFILE_LITE` (nhịp học + trình độ nếu thiếu), người tự đăng ký đi trọn phễu. Client
 * KHÔNG suy đoán từ `orgId`/gói ORG — chỉ tin `accountSource`.
 *
 * Bản mobile 1:1: `mobile/lib/onboardingContext.ts`. Sửa một bên thì sửa bên kia.
 */
