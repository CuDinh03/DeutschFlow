/**
 * Hợp đồng máy trạng thái onboarding v3.1 — chạy trên CHÍNH fixture dùng chung với mobile
 * (`docs/onboarding-flow-spec.transitions.json` ở repo root). Mobile có `onboardingMachine.test.ts`
 * chạy y hệt. Đổi luồng = sửa fixture trước, hai bên đỏ, rồi mới sửa code.
 */
...
// frontend/src/features/onboarding → repo root
...
describe("onboarding machine — fixture dùng chung web/mobile", () => {
  ...
  it.each(fixture.rows.map((r) => [r.id, r]))("%s", (_id, row) => {
  ...
  it("isZeroLevel: null/rỗng/A0 (mọi hoa thường) là ZERO, còn lại không", () => {
    expect(isZeroLevel(null)).toBe(true);
    expect(isZeroLevel(undefined)).toBe(true);
