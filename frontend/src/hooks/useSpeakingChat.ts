...
export function mapResponseToBubble(meta: AiChatResponse, isStreaming?: boolean): AiMessageBubble {
  return {
    ...
    adaptive: meta.adaptive
      ...
      : undefined,
    isStreaming,
    isSessionEnded: meta.isSessionEnded,
  };
}
...
export function useSpeakingChat(opts: {
        ...
        (errStr) => {
          void (async () => {
            ...
            try {
              ...
              if (res.data.aiSpeechDe) {
                speakGerman(res.data.aiSpeechDe, () => {
                  if (res.data.isSessionEnded) {
                    setShouldAutoEnd(true);
                  }
                });
              } else if (res.data.isSessionEnded) {
                setShouldAutoEnd(true);
              }
  ...
  }, [shouldAutoEnd, handleEndSession]);
