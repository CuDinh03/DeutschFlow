...
// Shared helpers for the per-class course screens (tc-checklist + tc-progress).
...
/** Loads the teacher's classes and tracks the selected class id (defaults to first). */
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        setClasses(list)
        setClassId((prev) => prev ?? list[0]?.id ?? null)
      })
