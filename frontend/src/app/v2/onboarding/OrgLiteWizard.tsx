  ...
  return (
    <div className="mx-auto w-full max-w-lg overflow-x-clip space-y-4" data-testid="org-lite-wizard">
      <section className="rounded-ga border border-ga-gold bg-ga-yellow-soft p-4" aria-labelledby="org-lite-heading">
        <p className="ga-ui text-ga-eyebrow uppercase text-ga-muted">{t("orgLite.cap")}</p>
        <h1 id="org-lite-heading" className="mt-1 font-ga-display text-ga-h1-m text-ga-ink">
          {className ? t("orgLite.heading", { className }) : t("orgLite.headingNoClass", { orgName })}
        </h1>
        {className && orgName ? (
          <p className="ga-ui mt-1 text-ga-small font-semibold text-ga-ink">{t("orgLite.orgLine", { orgName })}</p>
        ) : null}
        <p className="mt-2 text-ga-small text-ga-muted">{t("orgLite.sub")}</p>
        {trialUntil ? <p className="mt-2 text-ga-caption text-ga-ink">{t("orgLite.trialUntil", { date: trialUntil })}</p> : null}
      </section>

      {preset === null ? (
        <section className={card} aria-labelledby="org-lite-level">
          <h2 id="org-lite-level" className="font-ga-display text-ga-h2 text-ga-ink">{t("orgLite.levelHeading")}</h2>
          <p className="text-ga-small text-ga-muted">{t("orgLite.levelSub")}</p>
          {LITE_LEVELS.map((l) => (
            <button key={l.value} type="button" aria-pressed={currentLevel === l.value} onClick={() => setCurrentLevel(l.value)} className={sel(currentLevel === l.value)}>
              ...
              <div className="min-w-0 flex-1">
                <p className="ga-ui text-ga-small font-bold text-ga-ink">{t(`level.${l.value}.label`)}</p>
                <p className="text-ga-caption text-ga-muted">{t(`level.${l.value}.desc`)}</p>
              </div>
        ...
        <p className="ga-ui text-ga-small text-ga-muted px-1">{t("orgLite.levelPreset", { level: preset })}</p>
      )}

      <section className={card} aria-labelledby="org-lite-pace">
        <h2 id="org-lite-pace" className="font-ga-display text-ga-h2 text-ga-ink">{t("orgLite.paceHeading")}</h2>
        {WEEKLY.map((w) => (
          <button key={w.value} type="button" aria-pressed={weeklyTarget === w.value} onClick={() => setWeeklyTarget(w.value)} className={sel(weeklyTarget === w.value)}>
            ...
            <div className="min-w-0 flex-1">
              <p className="ga-ui text-ga-small font-bold text-ga-ink">{t(`pace.w${w.value}.label`)}</p>
              <p className="text-ga-caption text-ga-muted">{t(`pace.w${w.value}.desc`)}</p>
            </div>
          </button>
        ))}
      </section>
