...
export default function TeacherMaterialsPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Helper: xử lý khi job hoàn thành
      const handleCompleted = (payloadStr: string) => {
        try {
      ...
      // Polling fallback cho race condition
        ...
        pollTimer = setInterval(async () => {
          try {
            ...
            if (j.status === "COMPLETED") {
              ...
              pollTimer = null;
              esRef.current?.close();
              handleCompleted(j.resultPayload ?? "{}");
      ...
      // Khởi động polling sau 1 giây để bắt race condition
      setTimeout(() => {
