...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const userData: AuthUser = meRes.data;
        ...
        const [planRes, dashRes, skillsRes, reviewRes, todayRes] = await Promise.all([
          api.get("/plan/me"),
  ...
  return (
    ...
    >
          {apiError && (
            <div className="mb-6 df-glass-subtle rounded-2xl border border-red-200/90 bg-red-50/90 px-4 py-3 flex flex-wrap items-center justify-between gap-3 text-red-800 text-sm">
              <span>{apiError}</span>
              <button
                type="button"
                onClick={() => setReloadKey((k) => k + 1)}
                className="shrink-0 rounded-xl bg-[#00305E] px-4 py-2 text-xs font-semibold text-white hover:bg-[#004080] transition-colors"
              >
                {t("retry")}
              </button>
            </div>
          ...
          {/* Today's tasks — Error review (urgent) vs Learning / adaptive (progress) */}
          <div className="mb-6">
            <h2 className="text-lg font-bold text-[#1A1A1A] mb-3 tracking-tight">{t("todayTasksSectionTitle")}</h2>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="df-glass-subtle rounded-[18px] border-2 border-amber-400/35 bg-gradient-to-br from-amber-50/95 to-white/90 p-5 shadow-md shadow-amber-900/5">
                <div className="flex items-start gap-3 mb-4">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-amber-500/15 border border-amber-400/40">
                    <AlertTriangle className="text-amber-600" size={22} strokeWidth={2.2} />
                  </div>
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-wide text-amber-800/90">{t("todayTasksErrorBadge")}</p>
                    <h3 className="text-base font-bold text-[#1A1A1A]">{t("todayTasksErrorReviewTitle")}</h3>
                    <p className="text-sm text-[#64748B] mt-0.5">{t("todayTasksErrorReviewSubtitle")}</p>
                  </div>
                </div>
                {reviewCompleteErr && <p className="text-sm text-red-600 mb-3">{reviewCompleteErr}</p>}
                <button
                  type="button"
                  onClick={() => router.push("/speaking")}
                  className="mb-4 w-full sm:w-auto text-sm font-semibold text-[#B45309] flex items-center justify-center sm:justify-start gap-1 rounded-xl border border-amber-300/60 bg-white/60 px-3 py-2 hover:bg-white"
                >
                ...
                {reviewTasksToday.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {reviewTasksToday.map((task) => (
                      <li
                        key={task.id}
                        className="flex flex-col gap-2 rounded-[14px] border border-amber-200/80 bg-white/80 px-3 py-2.5 text-sm shadow-sm"
                      >
                        <div className="flex flex-wrap items-center gap-2 min-w-0">
                          <span className="font-semibold text-[#92400E]">{task.errorCode}</span>
                          <span className="text-[#64748B] text-xs">
                            {task.taskType === "REWRITE" ? t("reviewTaskTypeRewrite") : task.taskType}
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-[#00305E] text-white hover:bg-[#004080] disabled:opacity-50"
                          >
                          ...
                          <button
                            ...
                            className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-200 text-[#475569] hover:bg-white"
                          >
                            {t("reviewTaskNotYet")}
                          </button>
                          <button
                            ...
                            className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-amber-300/80 text-amber-900 hover:bg-amber-50"
                          >
              ...
              <div className="df-glass-subtle rounded-[18px] border-2 border-sky-500/30 bg-gradient-to-br from-sky-50/95 to-indigo-50/40 p-5 shadow-md shadow-sky-900/5">
                <div className="flex items-start gap-3 mb-4">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-sky-500/12 border border-sky-400/35">
                    <Sparkles className="text-sky-600" size={22} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-bold uppercase tracking-wide text-sky-800/90">{t("todayTasksLearningBadge")}</p>
                    <h3 className="text-base font-bold text-[#1A1A1A]">{t("todayTasksLearningTitle")}</h3>
                    <p className="text-sm text-[#64748B] mt-0.5">{t("todayTasksLearningSubtitle")}</p>
                  </div>
                </div>

                {todayPlan ? (
                  <>
                    <p className="text-sm text-[#334155] mb-3 leading-snug">
                      {t("todayHeroMeta", {
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-xl border border-sky-300/80 bg-white/90 px-3 py-2 text-xs font-semibold text-[#0c4a6e] hover:bg-white disabled:opacity-50 shadow-sm"
                      >
                    ...
                    {adaptiveFeedback && (
                      <p className="text-xs text-amber-800 mb-3 leading-snug bg-amber-50/80 border border-amber-200/60 rounded-xl px-3 py-2">
                        {adaptiveFeedback}
                      </p>
                    )}
                    <div className="grid gap-2 sm:grid-cols-2">
                      <div className="rounded-xl border border-white/60 bg-white/70 p-3">
                        <p className="text-[10px] font-semibold text-[#64748B] uppercase mb-1">{t("todayHeroRepair")}</p>
                        {todayPlan.dueRepairTasks.length === 0 ? (
                          <p className="text-xs text-[#64748B]">{t("todayHeroRepairEmpty")}</p>
                        ) : (
                          <ul className="space-y-1 text-xs font-mono text-[#0e7490]">
                            {todayPlan.dueRepairTasks.slice(0, 4).map((task) => (
                              <li key={task.id} className="truncate" title={task.errorCode}>
                                {task.errorCode}
                              </li>
                            ))}
                          </ul>
                      ...
                      <div className="rounded-xl border border-white/60 bg-white/70 p-3 flex flex-col">
                        <p className="text-[10px] font-semibold text-[#64748B] uppercase mb-1">{t("todayHeroSpeaking")}</p>
                        <p className="text-xs font-medium text-[#1e293b] line-clamp-2">{todayPlan.recommendedSpeaking.topic ?? "—"}</p>
                        <button
                          type="button"
                          onClick={() => router.push(todayPlan.recommendedSpeaking.href)}
                          className="mt-auto pt-2 text-left text-xs font-bold text-sky-700 flex items-center gap-0.5"
                        >
                          {t("todayHeroOpen")} <ChevronRight size={12} />
                        </button>
                      </div>
                      <div className="rounded-xl border border-white/60 bg-white/70 p-3 flex flex-col">
                        <p className="text-[10px] font-semibold text-[#64748B] uppercase mb-1">{t("todayHeroWeekly")}</p>
                      ...
                      <div className="rounded-xl border border-white/60 bg-white/70 p-3 flex flex-col">
                        <p className="text-[10px] font-semibold text-[#64748B] uppercase mb-1">{t("todayHeroVocab")}</p>
                        <p className="text-xs font-medium text-[#1e293b] line-clamp-2">{todayPlan.recommendedVocabPractice.topic ?? "—"}</p>
                        <button
                          type="button"
                  ...
                  <div className="rounded-xl border border-dashed border-sky-300/60 bg-white/50 px-4 py-6 text-center">
                    <p className="text-sm text-[#64748B] mb-3">{t("todayTasksLearningEmpty")}</p>
                    <button
                      type="button"
                      onClick={() => router.push("/roadmap")}
                      className="text-sm font-semibold text-sky-700 hover:underline"
                    >
                      {t("todayTasksLearningCta")}
                    </button>
                  </div>
                )}
              </div>
