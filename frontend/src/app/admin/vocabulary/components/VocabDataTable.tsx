...
import { ChevronLeft, ChevronRight, Image, Loader2, Pencil, RotateCcw, Sparkles } from 'lucide-react'
...
type VocabDataTableProps = {
  ...
  onReviewWord: (word: WordItem, e: React.MouseEvent) => void
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#F0F4F8] bg-[#FAFBFC]">
                {['ID', 'Ảnh', 'Từ', 'Loại', 'Cấp', 'Giống', 'IPA', 'Nghĩa VI', 'Nghĩa EN', 'Ví dụ DE', ''].map(h => (
              ...
              {words.map((w, i) => {
                ...
                return (
                  <motion.tr key={w.id}
                    ...
                    <td className="px-3 py-2.5">
                      {w.imageUrl ? (
                        <div className="relative w-8 h-8 rounded-[6px] overflow-hidden border border-[#E2E8F0] bg-[#F5F7FA] shrink-0">
                        ...
                        <div className="w-8 h-8 rounded-[6px] border border-dashed border-[#E2E8F0] flex items-center justify-center text-[#CBD5E1]">
                          <Image size={12} />
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-3 py-2.5" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={(e) => onReviewWord(w, e)} title="Review images"
                          className="p-1.5 rounded-[6px] hover:bg-indigo-50 text-[#94A3B8] hover:text-indigo-600 transition-colors">
                          <Sparkles size={13} />
                        </button>
