...
export default function AdminOverviewPage() {
  ...
  const systemHealth = ov
    ? (enrichPct >= 80 && ov.avgQuizScore >= 7 ? 'PASS' : enrichPct >= 60 ? 'ATTENTION' : 'CRITICAL')
    : 'UNKNOWN'

  const nextActions = ov ? [
    {
      title: 'Tăng độ phủ từ vựng',
      detail: `Còn ${ov.wordsNeedingEnrichment.toLocaleString()} từ cần bổ sung nghĩa/metadata.`,
      href: '/admin/vocabulary',
    },
    {
      title: 'Kiểm tra chất lượng bài quiz',
      detail: `Điểm quiz trung bình hiện tại là ${ov.avgQuizScore ? ov.avgQuizScore.toFixed(1) : '—'}.`,
      href: '/admin/reports',
    },
    {
      title: 'Rà user / class đang hoạt động',
      detail: 'Ưu tiên kiểm tra cohort có dấu hiệu tụt tiến độ hoặc không hoàn thành nhiệm vụ.',
      href: '/admin/users',
    },
  ] : []
    ...
    <AdminShell title={t('title')} subtitle={t('subtitle')}
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-[16px] p-5 border border-[#E2E8F0] shadow-sm border-l-4" style={{ borderLeftColor: systemHealth === 'PASS' ? '#10b981' : systemHealth === 'ATTENTION' ? '#f59e0b' : '#ef4444' }}>
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                <p className="text-xs uppercase tracking-widest text-[#94A3B8]">Actionable overview</p>
                <h2 className="text-lg font-extrabold text-[#0F172A] mt-1">System health: {systemHealth}</h2>
                <p className="text-sm text-[#64748B] mt-1">Mục tiêu hiện tại là giữ learning loop khỏe, tăng độ phủ dữ liệu và giảm ma sát vận hành.</p>
              </div>
              <a href={nextActions[0]?.href ?? '/admin/reports'} className="inline-flex items-center justify-center rounded-full bg-[#121212] text-white font-bold px-4 py-2 text-sm hover:opacity-90 transition-opacity">
                Đi tới hành động tiếp theo
              </a>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {nextActions.map((action) => (
              <a key={action.title} href={action.href} className="bg-[#F8FAFC] rounded-[16px] p-5 border border-[#E2E8F0] hover:border-[#121212]/20 hover:shadow-md transition-all">
                <p className="text-xs font-bold uppercase tracking-widest text-[#94A3B8] mb-2">Next action</p>
                <h3 className="text-base font-extrabold text-[#0F172A] mb-2">{action.title}</h3>
                <p className="text-sm text-[#64748B] leading-relaxed">{action.detail}</p>
              </a>
            ))}
          </div>
