...
import { getMediaByTag } from '@/lib/mediaApi'
...
export default function HomePage() {
  ...
  // Media S3 dynamic urls state
  const [heroUrl, setHeroUrl] = useState('/screen-dashboard.png')
  const [feature1Url, setFeature1Url] = useState('/img-6629.png')
  const [feature2Url, setFeature2Url] = useState('/erro-report.png')
  ...
  useEffect(() => {
    ...
    // Fetch landing page images dynamically from S3
    const loadLandingImages = async () => {
      const hero = await getMediaByTag('LANDING', 'hero')
      if (hero) setHeroUrl(hero.url)
      
      const feat1 = await getMediaByTag('LANDING', 'feature1')
      if (feat1) setFeature1Url(feat1.url)
      
      const feat2 = await getMediaByTag('LANDING', 'feature2')
      if (feat2) setFeature2Url(feat2.url)
    }
    loadLandingImages()
  }, [router])
    ...
    <div className="min-h-screen bg-white selection:bg-blue-100 selection:text-blue-900">
      ...
      <section className="pt-40 pb-20 px-6 overflow-hidden relative">
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          ...
          >
            <div className="w-full rounded-2xl md:rounded-[2rem] border-8 border-gray-900 shadow-2xl overflow-hidden">
              <MarketingImage
                src={heroUrl}
                alt={tMedia('landingHeroAlt')}
                priority
              />
            </div>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto space-y-32">
          ...
          <div className="flex flex-col md:flex-row items-center gap-16">
            ...
            <div className="flex-1 relative">
              ...
              {/* Mobile Mockup — tỷ lệ 9:20 giữ nguyên ảnh */}
              <div className="relative mx-auto w-[280px] rounded-[2.5rem] border-[10px] border-gray-900 shadow-2xl overflow-hidden">
                <MarketingImage
                  src={feature1Url}
                  alt={tMedia('landingFeature1Alt')}
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row-reverse items-center gap-16">
            ...
            <div className="flex-1 w-full">
              {/* Dashboard Report — tỷ lệ 2.4:1, dùng w-full h-auto */}
              <div className="max-w-xl mx-auto rounded-3xl border border-gray-200 shadow-xl overflow-hidden">
                <MarketingImage
                  src={feature2Url}
                  alt={tMedia('landingFeature2Alt')}
                />
              </div>
            </div>
          </div>
