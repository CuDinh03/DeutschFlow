        ...
        <div className="flex flex-col gap-3">
          {items.map((item, idx) => {
            ...
            return (
              ...
              >
                {item.imageUrl ? (
                  <div className="w-12 h-12 rounded-lg overflow-hidden border border-[#E2E8F0] shrink-0 bg-[#F8FAFC]">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={item.imageUrl} alt="" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).parentElement!.style.display = 'none' }} />
                  </div>
                ) : (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold border shrink-0" style={{ background: style.bg, color: style.text, borderColor: style.border }}>
                  {item.article}
                </span>
                )}
