...
export function mapWordListItemToCard(
  it: WordListItem & { category?: string },
): VocabCardItem {
  const gender = (it.gender ?? 'DER') as GenderCode
  const article: ArticleLower =
    it.article ??
    (gender === 'DER' ? 'der' : gender === 'DIE' ? 'die' : 'das')
  return {
    id: it.id,
    dtype: it.dtype,
    word: it.baseForm,
    ...
    gender,
    meaning: it.meaning ?? '',
    english: it.meaningEn ?? '',
    phonetic: it.phonetic ?? '',
    usageNote: it.usageNote ?? '',
    category: it.category ?? 'Alltag',
    level: it.cefrLevel,
    tags: it.tags ?? [],
    exampleDE: it.exampleDe ?? it.example ?? '',
    exampleTranslation: it.meaning ?? '',
    imageUrl: it.imageUrl ?? null,
    nounDetails: it.nounDetails,
    verbDetails: it.verbDetails,
    adjectiveDetails: it.adjectiveDetails,
  }
}
