...
export default function AssignmentDetailPage() {
  ...
  const handleStartSpeakingAI = async () => {
    ...
    try {
      ...
      // 2. Create Session
      const fullTopic = `Chủ đề: ${scenario.topic}\n\nMô tả chi tiết: ${scenario.scenarioDescription}\n\nGợi ý: ${scenario.followUpQuestions}`;
      const sessionRes = await aiSpeakingApi.createSession(
        fullTopic,
        scenario.level,
        "DEFAULT",
        "V1",
        ...
        null,
        null,
        assignment.id // This is StudentAssignment.id
      );
      ...
      console.error("Lỗi khi tạo phiên Luyện nói AI:", error);
      toast.error("Không thể bắt đầu phiên nói. Vui lòng thử lại.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          ...
          <div className="prose prose-sm max-w-none text-slate-600 mb-8 whitespace-pre-wrap">
            ...
            {assignment.attachmentUrl && (
              <div className="mt-4">
                ...
                >
                  <FileText size={18} /> Xem tài liệu đính kèm
                </a>
              </div>
            )}
          </div>
          ...
          {assignment.status === "PENDING" ? (
            assignment.assignmentType === "SPEAKING_SCENARIO" ? (
              <div className="bg-indigo-50 rounded-xl p-8 border border-indigo-100 text-center flex flex-col items-center">
                <h3 className="text-xl font-bold text-indigo-900 mb-3 flex items-center justify-center gap-2">
                  <UploadCloud size={24} className="text-indigo-500" /> Bài tập Luyện Nói AI
                </h3>
                <p className="text-slate-600 max-w-md mx-auto mb-6">
                  Bạn sẽ giao tiếp trực tiếp với AI Tutor theo kịch bản mà giáo viên đã thiết lập. Điểm số sẽ được tự động cập nhật sau khi bạn hoàn thành cuộc trò chuyện.
                </p>
                <button
                  onClick={handleStartSpeakingAI}
                  disabled={submitting}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-lg"
                >
                  {submitting && <Loader2 size={20} className="animate-spin" />}
                  🚀 Bắt đầu Luyện Nói AI
                </button>
              </div>
            ) : (
            ...
            <div className="space-y-6">
              ...
              {(assignment.status === "EVALUATED" || assignment.status === "GRADED") && (
                <div className="bg-emerald-50 rounded-xl p-6 border border-emerald-100">
                  <h3 className="text-lg font-bold text-emerald-900 mb-3 flex items-center gap-2">
                    {assignment.status === "GRADED" ? "Đánh giá từ AI" : "Nhận xét từ giáo viên"}
                  </h3>
