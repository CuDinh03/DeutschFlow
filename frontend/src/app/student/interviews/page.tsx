...
export default function InterviewsHistoryPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.get("/ai-speaking/sessions?size=50&sort=startedAt,desc"),
    ]).then(([sessRes]) => {
  ...
  const openSession = async (sess: SpeakingSession) => {
    ...
    try {
      ...
      const mapped: SessionMessage[] = raw.map((m: any) => ({
        ...
        contentDe: m.role?.toLowerCase() === "user" ? (m.userText ?? m.userMessage) : m.aiSpeechDe,
        isStreaming: false,
        feedback: {
          errors: m.errors || [],
  ...
  return (
    <StudentShell
      ...
      roadmapMeta={roadmapMeta ? {
        roadmapVersion: roadmapMeta.roadmapVersion ?? null,
        roadmapType: roadmapMeta.roadmapType ?? null,
        entryNodeCode: roadmapMeta.entryNodeCode ?? null,
        currentLevel: roadmapMeta.currentLevel ?? null,
        targetLevel: roadmapMeta.targetLevel ?? null,
        currentNodeCode: roadmapMeta.currentNodeCode ?? null,
        completedNodes: roadmapMeta.completedNodes ?? null,
        totalNodes: roadmapMeta.totalNodes ?? null,
        progressPercent: roadmapMeta.progressPercent ?? null,
        progressModel: roadmapMeta.progressModel ?? null,
      } : null}
    >
