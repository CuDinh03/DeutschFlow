...
export default function ReviewPage() {
  ...
  const [fromSpeaking, setFromSpeaking] = useState(false);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await api.get<ReviewCardData[]>('/srs/due');
        const data = res.data ?? [];
        setCards(data);
        if (data.length === 0) {
          setDone(true);
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-5">
        ...
        {sourceErrors.length > 0 && (
          <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 space-y-2">
            <p className="text-xs font-bold uppercase tracking-widest text-amber-700">Ôn lỗi từ speaking</p>
            <p className="text-sm text-amber-800 font-medium">{sourceLabel}</p>
            <div className="flex flex-wrap gap-2">
              {sourceErrors.map((err) => (
                <span key={err} className="text-xs px-2 py-1 rounded-full bg-white text-amber-700 border border-amber-200">
                  {err}
                </span>
              ))}
            </div>
