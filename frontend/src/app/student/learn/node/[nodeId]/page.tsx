...
export default function LearnNodePage() {
  ...
  const [roadmapState, setRoadmapState] = useState<{ nodeId: number; title: string; index: number; total: number; percent: number; nextNodeId: number | null; nextNodeTitle: string | null } | null>(null);

  const progressSummary = useMemo(() => {
    if (!roadmapState) return null;
    return `${roadmapState.index}/${roadmapState.total}`;
  }, [roadmapState]);
  ...
  useEffect(() => {
    ...
    const loadRoadmapState = async () => {
      try {
        ...
        const idx = data.findIndex((node: { id: number; title?: string; titleVi?: string }) => node.id === nodeId);
        if (idx < 0) return;
        const current = data[idx] as { id: number; title?: string; titleVi?: string };
        const next = data[idx + 1] as { id: number; title?: string; titleVi?: string } | undefined;
        setRoadmapState({
          nodeId: current.id,
          title: current.titleVi ?? current.title ?? `Node ${current.id}`,
          index: idx + 1,
          total: data.length,
          percent: data.length > 0 ? Math.round(((idx + 1) / data.length) * 100) : 0,
          nextNodeId: next?.id ?? null,
          nextNodeTitle: next ? (next.titleVi ?? next.title ?? null) : null,
        });
      } catch {
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto px-4 py-4 space-y-4">
        ...
        <button
          type="button"
          onClick={() => router.push("/roadmap/tree")}
        ...
        {session && (
          ...
          >
            ...
            {roadmapState && (
              <div className="mt-3 rounded-xl bg-white/10 border border-white/10 px-3 py-2 text-white">
                <div className="flex items-center justify-between gap-2 text-[11px]">
                  <span className="font-semibold">{tLearn("roadmapContext")}</span>
                  <span className="text-white/60">{roadmapState.title}</span>
                </div>
                <div className="mt-2 flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-white/15 overflow-hidden">
                    <div className="h-full rounded-full bg-[#FFCD00]" style={{ width: `${roadmapState.percent}%` }} />
                  </div>
                  <span className="text-[11px] font-semibold whitespace-nowrap">Node {roadmapState.index}/{roadmapState.total}</span>
                </div>
                <p className="mt-1 text-[11px] text-white/60">
                  {roadmapState.percent}% {tLearn("completedInRoadmap")}
                </p>
              </div>
            )}
          </motion.div>
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-16 gap-4 bg-white rounded-3xl border-2 border-[#E2E8F0]">
            ...
            <button onClick={() => router.push("/roadmap/tree")}
        ...
        {showRecap && session && (
          <SessionRecap
            ...
            onBack={() => { setShowRecap(false); router.push("/roadmap/tree"); }}
            onNext={() => {
              setShowRecap(false);
              if (roadmapState?.nextNodeId) router.push(`/student/learn/node/${roadmapState.nextNodeId}`);
