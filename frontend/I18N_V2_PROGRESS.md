...
- ✅ `GaTopBar` chrome strings (2026-07-23): search placeholder/aria, role chips, help aria →
  `v2.shell.*`; plus `LanguageToggle` group aria + `GaSidebarToggle` hamburger aria.
- ✅ Shared default copy (2026-07-23): only `ErrorBanner` had hardcoded defaults → now
  `v2.common.errorTitle/errorLoadFailed` + reuses `common.retry`. `GaPageHdr`/`EmptyState`/
  `LoadingState`/`SkeletonRow` carry no default VN copy (all via props) — nothing to migrate.
- ✅ `NotificationBell` dropdown (2026-07-23): header/aria/loading/error/empty/footer →
  `v2.account.notifications.*` (shared with the notifications page).
...
- ✅ profile · ✅ payment
- ✅ notifications (2026-07-23) — page + bell dropdown share `v2.account.notifications.*`
  (the old "blocked on notification WIP" gate cleared when PR #241 merged 19/07)

### public legacy (out of the v2 tree, in scope)
- ✅ login (2026-07-23 — form was already wired via `auth`; maintenance banner migrated:
  `auth.maintenanceTitle/dismissAria`; the banner MESSAGE stays env-driven via
  `NEXT_PUBLIC_MAINTENANCE_MESSAGE` by design)
- ✅ register (2026-07-23 — last 2 hardcoded strings → `auth.phoneLabel/phoneHelp`)
- ⬜ home (`/`) — **`components/landing-v2/GaLanding.tsx` still has ~108 hardcoded VN strings**
  (full marketing copy). Deliberately split into its own PR: en/de need copywriting quality,
  not mechanical translation.
- pricing — n/a: no public pricing page exists (only legacy-authed `/student/pricing`,
  out of scope as deprecated tree)
- terms · privacy — already trilingual via their own per-page mechanism (see
  `project_legal_pages_trilingual`); not migrated to next-intl on purpose

### Known leftovers (spotted during QA, not yet migrated)
- Session-expired dialog ("Phiên đăng nhập đã hết hạn…" + OK) renders hardcoded VN over the
  Galerie shell — surfaced during 2026-07-23 nogate QA on `/v2/notifications`. Small, shared,
  worth folding into the next i18n batch (find via grep "Phiên đăng nhập đã hết hạn").
